...
class EcosystemAPIAuditor:
        ...
        for endpoint in config_endpoints:
            ...
            if success and response:
                config_results["config_endpoints"].append(
                    {"endpoint": endpoint, "status": status, "has_config_data": self._contains_config_data(response)}
                )
    ...
    def identify_gaps(self) -> Dict[str, Any]:
        ...
        for service in self.services.keys():
            ...
            functional_endpoints = [
                ep for ep in api_endpoints if ep["success"] and ep["endpoint"] not in ["/health", "/", "/docs"]
            ]
            ...
            if functional_endpoints and service_gaps["cli_commands"] <= 2:  # Only health/status
                ...
                gaps_summary["available_but_not_cli"].append(
                    {"service": service, "endpoints": service_gaps["functional_gaps"]}
                )
    ...
    def generate_audit_report(self, gaps: Dict[str, Any]) -> None:
        ...
        for service in self.services.keys():
            ...
            config_status = (
                "✅"
                if config_analysis.get("cli_config_access")
                else "❌" if config_analysis.get("config_data_available") else "➖"
            )
