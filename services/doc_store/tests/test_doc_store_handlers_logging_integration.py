...
class TestDocStoreHandlersLoggingIntegration:
    ...
    async def setup_logger_client(self, mock_logger_client):
        ...
        with patch('services.doc_store.domain.analytics.handlers.get_logger_client') as mock_analytics_get_client, \
             patch('services.doc_store.domain.bulk.handlers.get_logger_client') as mock_bulk_get_client:
    ...
    async def test_document_analytics_retrieval_logging_success(self, mock_logger_client):
        ...
        with patch('services.doc_store.domain.analytics.handlers.AnalyticsService') as mock_service_class:
            ...
            retrieval_started = next((call for call in business_calls
                                    if call[0][0] == 'document_analytics_retrieval_started'), None)
            analytics_retrieved = next((call for call in business_calls
                                      if call[0][0] == 'document_analytics_retrieved'), None)
    ...
    async def test_bulk_document_creation_logging_success(self, mock_logger_client):
        ...
        mock_operation = type('MockOperation', (), {
            'id': 'bulk_op_12345',
        ...
        with patch('services.doc_store.domain.bulk.handlers.BulkOperationsService') as mock_service_class:
            ...
            creation_started = next((call for call in business_calls
                                   if call[0][0] == 'bulk_documents_creation_started'), None)
            documents_created = next((call for call in business_calls
                                    if call[0][0] == 'bulk_documents_created'), None)
    ...
    async def test_bulk_document_creation_logging_empty_request(self, mock_logger_client):
        ...
        with patch('services.doc_store.domain.bulk.handlers.BulkOperationsService') as mock_service_class:
            ...
            business_calls = mock_logger_client.log_business_event.call_args_list
            creation_rejected = next((call for call in business_calls
                                    if call[0][0] == 'bulk_documents_creation_rejected'), None)
    ...
    async def test_document_analytics_retrieval_logging_failure(self, mock_logger_client):
        ...
        with patch('services.doc_store.domain.analytics.handlers.AnalyticsService') as mock_service_class:
            ...
            retrieval_failed = next((call for call in business_calls
                                   if call[0][0] == 'document_analytics_retrieval_failed'), None)
    ...
    async def test_bulk_document_creation_logging_failure(self, mock_logger_client):
        ...
        with patch('services.doc_store.domain.bulk.handlers.BulkOperationsService') as mock_service_class:
            ...
            assert mock_logger_client.log_error.call_count == 1
            business_calls = mock_logger_client.log_business_event.call_args_list
            creation_failed = next((call for call in business_calls
                                  if call[0][0] == 'bulk_documents_creation_failed'), None)
    ...
    async def test_request_ids_generated_uniquely(self, mock_logger_client):
        ...
        mock_operation = type('MockOperation', (), {
            'id': 'bulk_op_test',
            'to_dict': lambda: {'id': 'bulk_op_test'}
        })()

        with patch('services.doc_store.domain.analytics.handlers.AnalyticsService') as mock_analytics_service_class, \
             patch('services.doc_store.domain.bulk.handlers.BulkOperationsService') as mock_bulk_service_class:
        ...
        for request_id in request_ids:
            assert any(prefix in request_id for prefix in [
                'doc_analytics_comprehensive_', 'bulk_doc_create_'
            ])
    ...
    async def test_logging_disabled_graceful_handling(self):
        ...
        with patch('services.doc_store.domain.analytics.handlers.get_logger_client') as mock_analytics_get_client, \
            ...
            with patch('services.doc_store.domain.analytics.handlers.AnalyticsService') as mock_analytics_service_class, \
                ...
                from services.doc_store.domain.bulk.handlers import BulkOperationsHandlers
                bulk_handler = BulkOperationsHandlers()
                result = await bulk_handler.handle_bulk_create_documents([
                    {"id": "doc1", "content": "Test content"}
                ])
                assert result["status"] == "success"  # Should still work without logging
    ...
    async def test_business_events_comprehensive_coverage(self, mock_logger_client):
        ...
        expected_events = {
            ...
            'document_analytics_retrieval_started', 'document_analytics_retrieved',
            'document_analytics_retrieval_failed',

            # Bulk operations events
            'bulk_documents_creation_started', 'bulk_documents_created',
            'bulk_documents_creation_rejected', 'bulk_documents_creation_failed'
        }
