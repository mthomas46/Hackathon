...
class LifecycleHandlers(BaseHandler):
    ...
    async def handle_update_policy(
    ...
    async def handle_evaluate_document_policies(
    ...
    async def handle_apply_lifecycle_policies(
        self, document: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if not lifecycle:
            return await self._handle_request(
                lambda: (_ for _ in ()).throw(
                    ValueError("Document lifecycle not found")
                )
            )
    ...
    async def handle_get_lifecycle_statistics(self) -> Dict[str, Any]:
        ...
        return await self._handle_request(
            lambda: stats, operation="get_lifecycle_statistics"
        )

    async def handle_update_policy_status(
