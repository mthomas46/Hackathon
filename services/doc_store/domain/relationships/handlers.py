...
class RelationshipsHandlers(BaseHandler):
    ...
    async def handle_get_relationships(
        ...
        result = self.service.get_relationships_for_document(
    ...
    async def handle_graph_statistics(self) -> Dict[str, Any]:
        ...
        return await self._handle_request(
            lambda: stats, operation="get_graph_statistics"
        )
    ...
    async def handle_build_graph(
        self, document_ids: Optional[list] = None, max_depth: int = 2
    ) -> Dict[str, Any]:
