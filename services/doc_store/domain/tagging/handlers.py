...
class TaggingHandlers(BaseHandler):
    ...
    async def handle_tag_document(self, document_id: str) -> Dict[str, Any]:
        ...
        if not document:
            return await self._handle_request(
                lambda: (_ for _ in ()).throw(ValueError("Document not found"))
            )

        # Tag the document
        tags = self.service.tag_document(
    ...
    async def handle_search_by_tags(
        ...
        if not tags:
            return await self._handle_request(
                lambda: (_ for _ in ()).throw(ValueError("Tags list cannot be empty"))
            )
    ...
    async def handle_create_taxonomy_node(
        self,
        tag: str,
        category: str,
        description: str = "",
        ...
        self._validate_request_data(
            {"tag": tag, "category": category}, ["tag", "category"]
        )

        node = self.service.create_taxonomy_node(
            tag, category, description, parent_tag, synonyms
        )
    ...
    async def handle_get_taxonomy_tree(
        self, root_category: Optional[str] = None
    ) -> Dict[str, Any]:
        """Handle taxonomy tree retrieval."""
        tree = self.service.get_taxonomy_tree(root_category)

        return await self._handle_request(
            lambda: tree, operation="get_taxonomy_tree", root_category=root_category
        )
    ...
    async def handle_get_tag_statistics(self) -> Dict[str, Any]:
        ...
        return await self._handle_request(lambda: stats, operation="get_tag_statistics")

    async def handle_remove_document_tags(
    ...
    async def handle_update_tag_confidence(
