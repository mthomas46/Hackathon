...
class TaggingHandlers(BaseHandler):
    ...
    async def handle_search_by_tags(
        ...
        return await self._handle_request(
            lambda: result, operation="search_by_tags", searched_tags=tags, results_count=result["total"]
        )
    ...
    async def handle_create_taxonomy_node(
        ...
        self._validate_request_data({"tag": tag, "category": category}, ["tag", "category"])

        node = self.service.create_taxonomy_node(tag, category, description, parent_tag, synonyms)

        return await self._handle_request(
            lambda: node.to_dict(), operation="create_taxonomy_node", tag=tag, category=category
        )
    ...
    async def handle_get_taxonomy_tree(self, root_category: Optional[str] = None) -> Dict[str, Any]:
        ...
        return await self._handle_request(lambda: tree, operation="get_taxonomy_tree", root_category=root_category)
    ...
    async def handle_update_tag_confidence(self, document_id: str, tag: str, confidence: float) -> Dict[str, Any]:
        ...
        if not (0 <= confidence <= 1):
            return await self._handle_request(
                lambda: (_ for _ in ()).throw(ValueError("Confidence must be between 0 and 1"))
            )
