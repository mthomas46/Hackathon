...
class TeamMemberProfile(BaseModel):
    ...
    specialization: List[str] = Field(
        default_factory=list, description="Technical specializations"
    )
    productivity_multiplier: float = Field(
        1.0, description="Productivity factor (0.5-2.0)"
    )
...
class TimelinePhase(BaseModel):
    ...
    dependencies: List[str] = Field(
        default_factory=list, description="Prerequisite phases"
    )
    team_allocation: Dict[TeamRole, int] = Field(
        default_factory=dict, description="Team members needed by role"
    )
...
class SimulationParameters(BaseModel):
    ...
    speed_multiplier: float = Field(
        1.0, description="Speed up simulation (1.0 = real-time)"
    )
    enable_real_time_feed: bool = Field(
        True, description="Enable real-time event streaming"
    )
    enable_analysis: bool = Field(
        True, description="Run analysis workflows during simulation"
    )
    enable_notifications: bool = Field(
        False, description="Send notifications during simulation"
    )
    max_concurrent_workflows: int = Field(
        5, description="Maximum concurrent workflow executions"
    )
    output_directory: str = Field(
        "./simulation_output", description="Directory for output files"
    )
...
class SimulationEvent(BaseModel):
    ...
    entity_id: Optional[str] = Field(
        None, description="ID of related entity (document, ticket, etc.)"
    )
...
class SimulationProgress(BaseModel):
    ...
    current_time: datetime = Field(
        default_factory=datetime.now, description="Current simulation time"
    )
...
class TicketMetadata(BaseModel):
    ...
    comments: List[Dict[str, Any]] = Field(
        default_factory=list, description="Ticket comments"
    )
...
class PRMetadata(BaseModel):
    ...
    id: str = Field(
        default_factory=lambda: str(UUID.uuid4()), description="Unique PR ID"
    )
    ...
    commits: List[Dict[str, Any]] = Field(
        default_factory=list, description="PR commits"
    )
    reviews: List[Dict[str, Any]] = Field(
        default_factory=list, description="PR reviews"
    )
    comments: List[Dict[str, Any]] = Field(
        default_factory=list, description="PR comments"
    )
...
class DocumentTemplate(BaseModel):
    ...
    content_structure: Dict[str, Any] = Field(
        ..., description="Content structure template"
    )
...
class ProjectTemplate(BaseModel):
    ...
    team_composition: Dict[TeamRole, int] = Field(
    ...
    technologies: List[str] = Field(
        default_factory=list, description="Default technologies"
    )
