...
class TestRunner:
    ...
    def run_all_tests(
        self, parallel: bool = False, coverage: bool = True, verbose: bool = False
    ) -> int:
        """Run all tests."""
        ...
        cmd = self._build_pytest_command(
            markers=None, parallel=parallel, coverage=coverage, verbose=verbose
        )
    ...
    def run_unit_tests(
        ...
        cmd = self._build_pytest_command(
            markers=["unit"], parallel=parallel, coverage=coverage, verbose=verbose
        )
    ...
    def run_critical_tests(
        ...
        cmd = self._build_pytest_command(
            markers=["critical"], parallel=parallel, coverage=coverage, verbose=verbose
        )
    ...
    def run_ddd_foundation_tests(
        ...
        cmd = self._build_pytest_command(
            test_paths=test_files, parallel=parallel, coverage=coverage, verbose=verbose
        )
    ...
    def generate_coverage_report(self) -> None:
        ...
        if self.coverage_dir.exists():
            print(
                f"Coverage report available at: file://{self.coverage_dir}/index.html"
            )
        else:
    ...
    def _build_pytest_command(
        ...
        if parallel:
            try:
                ...
                print(
                    "⚠️  pytest-xdist not installed. Install with: pip install pytest-xdist"
                )
    ...
    def _execute_command(self, cmd: List[str]) -> int:
        ...
        try:
            ...
            result = subprocess.run(
                ...
                text=True,  # Show output in real-time
            )
...
Test Categories:
    ...
    parser.add_argument(
        "--domain", action="store_true", help="Run domain layer tests only"
    )

    parser.add_argument(
        "--critical", action="store_true", help="Run critical path tests only"
    )
    ...
    for test_func in test_functions:
        exit_code = test_func(
            parallel=args.parallel, coverage=args.coverage, verbose=args.verbose
        )
