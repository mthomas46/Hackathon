...
class SimulationErrorHandlingIntegration:
    ...
    async def _enhance_error_handling(
        self, error: Exception, context: ErrorContext, shared_result: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _assess_business_impact(self, error: Exception, context: ErrorContext) -> str:
        ...
        if error_type in ["connection", "authorization"] and context.operation in [
            "start_simulation",
            "save_results",
            "generate_reports",
        ]:
