...
class ContentContext:
    ...
    def _build_timeline_awareness(self) -> Dict[str, Any]:
        ...
        sum(phase.get("duration_days", 7) for phase in phases)
        elapsed_phases = 0
        current_phase = None
...
class ContextAwareDocumentGenerator:
    ...
    async def generate_context_aware_document(
        self,
        document_type: str,
        project_config: Dict[str, Any],
        team_members: List[Dict[str, Any]],
    ...
    async def _generate_section_content(
        self, section: str, template: Dict[str, Any], context: ContentContext, **kwargs
    ) -> str:
        ...
        try:
            response = await self.llm_client.generate_content(
                prompt=prompt, model="gpt-4", temperature=0.7, max_tokens=500
            )
...
"""
    ...
    def _find_dominant_personality(
        self, context: ContentContext, personality_weights: Dict[str, float]
    ) -> Optional[str]:
