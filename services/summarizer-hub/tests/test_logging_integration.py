...
class TestSummarizerHubLoggingIntegration:
    ...
    async def test_document_summarization_failure_logging(self, client, mock_logger_client):
        ...
        with patch("main.summarizer") as mock_summarizer:
            ...
            # Make request
            request_data = {"content": "Short content that should fail", "format": "markdown", "max_length": 200}
            ...
            summarization_error = next(
                (call for call in error_calls if "Document summarization failed" in call[0][0]), None
            )
    ...
    async def test_different_summarization_formats_logging(self, client, mock_logger_client):
        ...
        for fmt in formats:
            ...
            with patch("main.summarizer") as mock_summarizer:
                ...
                # Make request
                request_data = {"content": "Content to summarize", "format": fmt, "max_length": 300}
    ...
    async def test_compression_metrics_calculation_logging(self, client, mock_logger_client):
        ...
        original_content = (
            "This is a long document with many words that should be compressed significantly when summarized."
        )
        ...
        with patch("main.summarizer") as mock_summarizer:
            ...
            # Make request
            request_data = {"content": original_content, "format": "markdown", "max_length": 100}
    ...
    async def test_logging_disabled_graceful_handling(self, client):
        ...
        with patch("main.summarizer") as mock_summarizer:
            ...
            request_data = {"content": "Content to summarize", "format": "markdown", "max_length": 300}
    ...
    async def test_performance_metric_accuracy(self, client, mock_logger_client):
        ...
        with patch("main.summarizer") as mock_summarizer:
            ...
            # Make request
            request_data = {"content": "Content to be summarized", "format": "markdown", "max_length": 250}
            response = client.post("/summarize", json=request_data)
            assert response.status_code == 200
    ...
    async def test_empty_content_summarization_logging(self, client, mock_logger_client):
        ...
        with patch("main.summarizer") as mock_summarizer:
            ...
            request_data = {"content": "", "format": "markdown", "max_length": 100}
