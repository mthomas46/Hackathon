...
class SearchRegistryUseCase:
    """Use case for searching the registry."""
    ...
    async def search(
        ...
        """Search registry."""
        logger.info(f"Searching registry: query='{query}', tier={tier}, tags={tags}")
        
        entries = await self.registry_repo.search(query, tier, tags, limit)
