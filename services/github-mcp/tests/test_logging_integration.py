...
class TestGitHubMCPLoggingIntegration:
    ...
    async def test_tool_invocation_mock_mode_successful_logging(self, client, mock_logger_client):
        ...
        with patch("main.mock_implementations") as mock_impl, patch("main.event_system") as mock_events, patch(
            "main.config"
        ) as mock_config:
            ...
            completion_call = next(
                call for call in business_calls if call[0][0] == "github_mcp_tool_invocation_completed"
            )
    ...
    async def test_tool_invocation_upstream_proxy_successful_logging(self, client, mock_logger_client):
        ...
        with patch("main.config") as mock_config, patch("main.ServiceClients") as mock_clients_class:
            ...
            # Make request
            request_data = {"arguments": {"repo": "test/repo"}, "mock": False}
    ...
    async def test_tool_invocation_read_only_block_logging(self, client, mock_logger_client):
        ...
        with patch("main.config") as mock_config:
            ...
            request_data = {"arguments": {"title": "New Issue", "body": "Description"}, "write": True}
    ...
    async def test_tool_invocation_upstream_proxy_failure_logging(self, client, mock_logger_client):
        ...
        with patch("main.config") as mock_config, patch("main.ServiceClients") as mock_clients_class:
            ...
            # Make request
            request_data = {"arguments": {"repo": "test/repo"}}
    ...
    async def test_tool_invocation_local_execution_failure_logging(self, client, mock_logger_client):
        ...
        with patch("main.mock_implementations") as mock_impl, patch("main.config") as mock_config:
            ...
            # Make request
            request_data = {"arguments": {"owner": "invalid", "repo": "nonexistent"}}
    ...
    async def test_different_execution_modes_logging(self, client, mock_logger_client):
        ...
        for mode_name, request_params, expected_mode in modes_and_results:
            with patch("main.mock_implementations") as mock_impl, patch(
                "main.real_implementations"
            ) as mock_real_impl, patch("main.config") as mock_config, patch("main.event_system") as mock_events:
                ...
                # Make request
                request_data = {"arguments": {"test": "data"}, **request_params}
    ...
    async def test_logging_disabled_graceful_handling(self, client):
        ...
        with patch("main.mock_implementations") as mock_impl, patch("main.config") as mock_config, patch(
            "main.event_system"
        ) as mock_events:
