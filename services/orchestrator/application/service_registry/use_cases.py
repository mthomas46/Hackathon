...
class ListServicesUseCase(UseCase):
    ...
    async def execute(self, query: ListServicesQuery) -> DomainResult[Dict[str, Any]]:
        ...
        try:
            ...
            # Convert Service objects to RegistryEntryResponse format
            registry_entries = []
            for service in paginated_services:
                entry = {
                    "service_name": service.name,
                    "service_url": service.base_url or "",
            ...
            result = {
                "services": registry_entries,
                "total": len(unique_services)
            }

            return DomainResult.success_result(result)
        except Exception as e:
