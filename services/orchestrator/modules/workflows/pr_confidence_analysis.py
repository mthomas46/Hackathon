...
class PRConfidenceAnalysisWorkflow:
    ...
    async def analyze_requirements_alignment_node(
        ...
        prompt = await get_optimal_prompt_tool(
            "requirements_alignment_analysis",
            {"task_type": "pr_review", "analysis_focus": "requirements_coverage"},
        )
    ...
    async def calculate_confidence_score_node(
        ...
        alignment_score = (
            state["context"].get("requirements_alignment", {}).get("overall_score", 0)
        )
        consistency_results = state["context"].get("documentation_consistency", [])

        # Calculate weighted confidence score
        consistency_avg = (
            sum(r.get("consistency_score", 0) for r in consistency_results)
    ...
    async def identify_gaps_and_risks_node(self, state: WorkflowState) -> WorkflowState:
        ...
        confidence_score = (
            state["context"].get("confidence_score", {}).get("overall_score", 0)
        )
        if confidence_score < 0.6:
            risks.append(
                "Low confidence score indicates significant implementation gaps"
            )
        elif confidence_score < 0.8:
            risks.append(
                "Medium confidence score suggests review required before approval"
            )
    ...
    async def generate_recommendations_node(
        ...
        for gap in gaps:
            if "test" in gap.lower():
                ...
                recommendations.append(
                    "Update documentation to address consistency issues"
                )
            else:
        ...
        if confidence_score >= 0.8:
            ...
            recommendations.append(
                "PR has significant gaps that must be addressed before approval"
            )
    ...
    async def create_final_report_node(self, state: WorkflowState) -> WorkflowState:
        ...
        report = {
            ...
            "summary": {
                "requirements_coverage": state["context"]
                .get("requirements_alignment", {})
                .get("overall_score", 0),
                "documentation_consistency": (
                    sum(
                        r.get("consistency_score", 0)
                        for r in state["context"].get("documentation_consistency", [])
                    )
            ...
            "approval_recommendation": (
                "approve"
                if confidence_result.get("overall_score", 0) >= 0.8
    ...
    async def send_notifications_node(self, state: WorkflowState) -> WorkflowState:
        ...
        await send_notification_tool(
            ...
            urgency=(
                "high" if confidence_result.get("overall_score", 0) < 0.6 else "normal"
            ),
            additional_data={
                "pr_id": pr_details.get("id"),
        ...
        state["messages"].append(
            {"role": "assistant", "content": "Notifications sent to stakeholders"}
        )
