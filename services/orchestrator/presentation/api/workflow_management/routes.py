...
async def create_workflow(
    ...
    try:
        ...
        if logger:
            await logger.log_business_event("workflow_creation_started", {
            ...
            await logger.log_info("Initiating workflow creation", {
                "request_id": request_id,
                "workflow_name": request.name,
                "workflow_type": request.workflow_type,
        ...
        if not result.success:
            ...
            if logger:
                await logger.log_business_event("workflow_creation_failed", {
                    "request_id": request_id,
                    "operation": "workflow_lifecycle_management",
                    "workflow_type": request.workflow_type,
                    "workflow_name": request.name,
        ...
        if logger:
            await logger.log_business_event("workflow_created", {
                "request_id": request_id,
        ...
        if logger:
            ...
            await logger.log_business_event("workflow_creation_error", {
                "request_id": request_id,
                "operation": "workflow_lifecycle_management",
                "workflow_type": request.workflow_type,
                "workflow_name": request.name,
...
async def execute_workflow(
    ...
    try:
        ...
        if request.workflow_id != workflow_id:
            ...
            if logger:
                await logger.log_business_event("workflow_execution_rejected", {
                    "request_id": request_id,
                    "workflow_id": workflow_id,
        ...
        if logger:
            ...
            await logger.log_info("Initiating workflow execution", {
                "request_id": request_id,
                "workflow_id": workflow_id,
                "execution_priority": request.priority,
        ...
        # Convert to response
        execution_id = result.execution.execution_id if hasattr(result.execution, 'execution_id') else result.execution.get("execution_id")
        response_time = time.time() - start_time

        # Log successful workflow execution initiation
        if logger:
            await logger.log_business_event("workflow_execution_initiated", {
                "request_id": request_id,
                "workflow_id": workflow_id,
        ...
        if logger:
            ...
            await logger.log_business_event("workflow_execution_error", {
                "request_id": request_id,
                "operation": "workflow_execution_control",
                "workflow_id": workflow_id,
                "execution_priority": request.priority,
...
async def get_workflow(
    ...
    try:
        ...
        if logger:
            await logger.log_business_event("workflow_retrieval_started", {
                "request_id": request_id,
                "workflow_id": workflow_id,
            ...
            await logger.log_info("Retrieving workflow definition", {
                "request_id": request_id,
                "workflow_id": workflow_id,
        ...
        if not result.success:
            ...
            if "not found" in result.error_message.lower():
                ...
                if logger:
                    await logger.log_business_event("workflow_not_found", {
                        "request_id": request_id,
                        "workflow_id": workflow_id,
                        "operation": "workflow_definition_access",
                        "response_time_seconds": response_time,
            ...
            if logger:
                await logger.log_business_event("workflow_retrieval_failed", {
                    "request_id": request_id,
                    "workflow_id": workflow_id,
                    "operation": "workflow_definition_access",
                    "response_time_seconds": response_time,
        ...
        if logger:
            await logger.log_business_event("workflow_retrieved", {
                "request_id": request_id,
                "workflow_id": workflow_id,
                "operation": "workflow_definition_access",
                "response_time_seconds": response_time,
        ...
        if logger:
            ...
            await logger.log_business_event("workflow_retrieval_error", {
                "request_id": request_id,
                "operation": "workflow_definition_access",
                "workflow_id": workflow_id,
                "error_type": type(e).__name__,
...
async def list_workflows(
    ...
    try:
        ...
        if logger:
            ...
            await logger.log_business_event("workflows_listing_started", {
                "request_id": request_id,
            ...
            await logger.log_info("Listing workflow definitions", {
                "request_id": request_id,
        ...
        if logger:
            await logger.log_business_event("workflows_listed", {
                "request_id": request_id,
                "operation": "workflow_inventory_management",
                "response_time_seconds": response_time,
                "success": True,
        ...
        if logger:
            ...
            await logger.log_business_event("workflows_listing_failed", {
                "request_id": request_id,
                "operation": "workflow_inventory_management",
                "error_type": type(e).__name__,
                "error_message": str(e),
...
async def list_executions(
    ...
    try:
        ...
        if logger:
            ...
            await logger.log_business_event("executions_listing_started", {
                "request_id": request_id,
            ...
            await logger.log_info("Listing workflow execution history", {
                "request_id": request_id,
                "pagination_limit": limit,
                "pagination_offset": offset,
                "filters_active": filters_applied,
        ...
        if not result.success:
            ...
            if logger:
                await logger.log_business_event("executions_listing_failed", {
                    "request_id": request_id,
                    "operation": "workflow_execution_history",
                    "response_time_seconds": response_time,
                    "error_message": result.error_message,
        ...
        if logger:
            await logger.log_business_event("executions_listed", {
                "request_id": request_id,
                "operation": "workflow_execution_history",
                "response_time_seconds": response_time,
                "success": True,
...
async def get_execution(
    ...
    try:
        ...
        if logger:
            await logger.log_business_event("execution_detail_started", {
                "request_id": request_id,
                "execution_id": execution_id,
            ...
            await logger.log_info("Retrieving workflow execution details", {
                "request_id": request_id,
                "execution_id": execution_id,
        ...
        if not result.success:
            ...
            if "not found" in result.error_message.lower():
                ...
                if logger:
                    await logger.log_business_event("execution_not_found", {
                        "request_id": request_id,
                        "execution_id": execution_id,
                        "operation": "workflow_execution_monitoring",
                        "response_time_seconds": response_time,
            ...
            if logger:
                await logger.log_business_event("execution_detail_failed", {
                    "request_id": request_id,
                    "execution_id": execution_id,
                    "operation": "workflow_execution_monitoring",
                    "response_time_seconds": response_time,
        ...
        if logger:
            await logger.log_business_event("execution_detail_retrieved", {
                "request_id": request_id,
                "execution_id": execution_id,
                "operation": "workflow_execution_monitoring",
                "response_time_seconds": response_time,
        ...
        if logger:
            ...
            await logger.log_business_event("execution_detail_error", {
                "request_id": request_id,
                "operation": "workflow_execution_monitoring",
                "execution_id": execution_id,
                "error_type": type(e).__name__,
