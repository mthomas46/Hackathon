...
class TestOrchestratorWorkflowManagementLoggingIntegration:
    ...
    async def setup_logger_client(self, mock_logger_client):
        ...
        with patch(
            "services.orchestrator.presentation.api.workflow_management.routes.get_logger_client"
        ) as mock_get_client:
    ...
    async def test_workflow_creation_logging_success(self, client, mock_logger_client):
        ...
        mock_result = type("MockResult", (), {"success": True, "workflow": mock_workflow})()
        ...
        with patch("services.orchestrator.presentation.api.workflow_management.routes.container") as mock_container:
            ...
            creation_started = next(
                (call for call in business_calls if call[0][0] == "workflow_creation_started"), None
            )
            workflow_created = next((call for call in business_calls if call[0][0] == "workflow_created"), None)
    ...
    async def test_workflow_execution_logging_success(self, client, mock_logger_client):
        ...
        mock_result = type("MockResult", (), {"success": True, "execution": mock_execution})()
        ...
        with patch("services.orchestrator.presentation.api.workflow_management.routes.container") as mock_container:
            ...
            execution_started = next(
                (call for call in business_calls if call[0][0] == "workflow_execution_started"), None
            )
            execution_initiated = next(
                (call for call in business_calls if call[0][0] == "workflow_execution_initiated"), None
            )
    ...
    async def test_workflow_retrieval_logging_success(self, client, mock_logger_client):
        ...
        with patch("services.orchestrator.presentation.api.workflow_management.routes.container") as mock_container:
            ...
            retrieval_started = next(
                (call for call in business_calls if call[0][0] == "workflow_retrieval_started"), None
            )
            workflow_retrieved = next((call for call in business_calls if call[0][0] == "workflow_retrieved"), None)
    ...
    async def test_workflows_listing_logging_with_filters(self, client, mock_logger_client):
        ...
        with patch("services.orchestrator.presentation.api.workflow_management.routes.container") as mock_container:
            ...
            business_calls = mock_logger_client.log_business_event.call_args_list
            listing_started = next((call for call in business_calls if call[0][0] == "workflows_listing_started"), None)
            workflows_listed = next((call for call in business_calls if call[0][0] == "workflows_listed"), None)
    ...
    async def test_executions_listing_logging_with_filters(self, client, mock_logger_client):
        ...
        mock_result = type("MockResult", (), {"success": True, "executions": mock_executions, "total": 1})()
        ...
        with patch("services.orchestrator.presentation.api.workflow_management.routes.container") as mock_container:
            ...
            listing_started = next(
                (call for call in business_calls if call[0][0] == "executions_listing_started"), None
            )
            executions_listed = next((call for call in business_calls if call[0][0] == "executions_listed"), None)
    ...
    async def test_execution_detail_retrieval_logging_success(self, client, mock_logger_client):
        ...
        with patch("services.orchestrator.presentation.api.workflow_management.routes.container") as mock_container:
            ...
            business_calls = mock_logger_client.log_business_event.call_args_list
            detail_started = next((call for call in business_calls if call[0][0] == "execution_detail_started"), None)
            detail_retrieved = next(
                (call for call in business_calls if call[0][0] == "execution_detail_retrieved"), None
            )
    ...
    async def test_workflow_creation_logging_failure(self, client, mock_logger_client):
        ...
        mock_result = type(
            "MockResult",
            (),
            {"success": False, "error_message": "Invalid workflow configuration: missing required actions"},
        )()
        ...
        with patch("services.orchestrator.presentation.api.workflow_management.routes.container") as mock_container:
            ...
            # Check failure event
            business_calls = mock_logger_client.log_business_event.call_args_list
            workflow_failed = next((call for call in business_calls if call[0][0] == "workflow_creation_failed"), None)
    ...
    async def test_workflow_execution_logging_id_mismatch(self, client, mock_logger_client):
        ...
        # Verify logging calls
        business_calls = mock_logger_client.log_business_event.call_args_list
        execution_rejected = next(
            (call for call in business_calls if call[0][0] == "workflow_execution_rejected"), None
        )
    ...
    async def test_request_ids_generated_uniquely(self, client, mock_logger_client):
        ...
        with patch("services.orchestrator.presentation.api.workflow_management.routes.container") as mock_container:
            ...
            client.post("/api/v1/workflow-management", json={"name": "Test", "workflow_type": "test", "actions": []})
            client.post("/api/v1/workflow-management/test/execute", json={"workflow_id": "test", "parameters": {}})
