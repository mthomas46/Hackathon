...
class TestOrchestratorInfrastructureLoggingIntegration:
    ...
    async def test_saga_creation_logging_success(self, client, mock_logger_client):
        ...
        with patch('services.orchestrator.presentation.api.infrastructure.routes.container') as mock_container:
            ...
            saga_started = next((call for call in business_calls
                               if call[0][0] == 'saga_creation_started'), None)
            saga_created = next((call for call in business_calls
                               if call[0][0] == 'saga_created'), None)
    ...
    async def test_saga_creation_logging_failure(self, client, mock_logger_client):
        ...
        with patch('services.orchestrator.presentation.api.infrastructure.routes.container') as mock_container:
            ...
            # Check failure event
            business_calls = mock_logger_client.log_business_event.call_args_list
            saga_failed = next((call for call in business_calls
                              if call[0][0] == 'saga_creation_failed'), None)
    ...
    async def test_saga_retrieval_logging_success(self, client, mock_logger_client):
        ...
        with patch('services.orchestrator.presentation.api.infrastructure.routes.container') as mock_container:
            ...
            business_calls = mock_logger_client.log_business_event.call_args_list
            saga_retrieved = next((call for call in business_calls
                                 if call[0][0] == 'saga_retrieved'), None)
    ...
    async def test_saga_retrieval_logging_not_found(self, client, mock_logger_client):
        ...
        with patch('services.orchestrator.presentation.api.infrastructure.routes.container') as mock_container:
            ...
            # Verify logging calls
            business_calls = mock_logger_client.log_business_event.call_args_list
            saga_not_found = next((call for call in business_calls
                                 if call[0][0] == 'saga_not_found'), None)
    ...
    async def test_saga_listing_logging_success(self, client, mock_logger_client):
        ...
        mock_result = {
            "sagas": [
                {"saga_id": "saga-1", "status": "active"},
                {"saga_id": "saga-2", "status": "completed"}
            ],
            "total": 2
        }
        ...
        with patch('services.orchestrator.presentation.api.infrastructure.routes.container') as mock_container:
            ...
            listing_completed = next((call for call in business_calls
                                    if call[0][0] == 'saga_listing_completed'), None)
    ...
    async def test_trace_creation_logging_success(self, client, mock_logger_client):
        ...
        with patch('services.orchestrator.presentation.api.infrastructure.routes.container') as mock_container:
            ...
            business_calls = mock_logger_client.log_business_event.call_args_list
            trace_created = next((call for call in business_calls
                                if call[0][0] == 'trace_created'), None)
    ...
    async def test_trace_retrieval_logging_success(self, client, mock_logger_client):
        ...
        with patch('services.orchestrator.presentation.api.infrastructure.routes.container') as mock_container:
            ...
            business_calls = mock_logger_client.log_business_event.call_args_list
            trace_retrieved = next((call for call in business_calls
                                  if call[0][0] == 'trace_retrieved'), None)
    ...
    async def test_dlq_stats_retrieval_logging_success(self, client, mock_logger_client):
        ...
        with patch('services.orchestrator.presentation.api.infrastructure.routes.container') as mock_container:
            ...
            stats_retrieved = next((call for call in business_calls
                                  if call[0][0] == 'dlq_stats_retrieved'), None)
    ...
    async def test_dlq_retry_operation_logging_success(self, client, mock_logger_client):
        ...
        with patch('services.orchestrator.presentation.api.infrastructure.routes.container') as mock_container:
            ...
            business_calls = mock_logger_client.log_business_event.call_args_list
            retry_completed = next((call for call in business_calls
                                  if call[0][0] == 'dlq_retry_completed'), None)
    ...
    async def test_event_publish_logging_success(self, client, mock_logger_client):
        ...
        with patch('services.orchestrator.presentation.api.infrastructure.routes.container') as mock_container:
            ...
            response = client.post("/api/v1/infrastructure/events/publish?event_type=test_event",
            ...
            business_calls = mock_logger_client.log_business_event.call_args_list
            event_published = next((call for call in business_calls
                                  if call[0][0] == 'event_published'), None)
    ...
    async def test_event_replay_logging_placeholder(self, client, mock_logger_client):
        ...
        business_calls = mock_logger_client.log_business_event.call_args_list
        replay_completed = next((call for call in business_calls
                               if call[0][0] == 'event_replay_completed'), None)
    ...
    async def test_event_clear_logging_placeholder(self, client, mock_logger_client):
        ...
        business_calls = mock_logger_client.log_business_event.call_args_list
        clear_completed = next((call for call in business_calls
                              if call[0][0] == 'event_clear_completed'), None)
    ...
    async def test_business_events_comprehensive_coverage(self, client, mock_logger_client):
        ...
        expected_events = {
            # Saga events
            'saga_creation_started', 'saga_created', 'saga_creation_failed',
            'saga_retrieval_started', 'saga_retrieved', 'saga_not_found', 'saga_retrieval_failed',
            'saga_listing_started', 'saga_listing_completed', 'saga_listing_failed',

            # Trace events
            'trace_creation_started', 'trace_created', 'trace_creation_failed',
            'trace_retrieval_started', 'trace_retrieved', 'trace_not_found', 'trace_retrieval_failed',
            'trace_listing_started', 'trace_listing_completed', 'trace_listing_failed',

            # DLQ events
            'dlq_stats_retrieval_started', 'dlq_stats_retrieved', 'dlq_stats_retrieval_failed',
            'dlq_events_listing_started', 'dlq_events_listed', 'dlq_events_listing_failed',
            'dlq_retry_started', 'dlq_retry_completed', 'dlq_retry_failed',

            # Event streaming events
            'event_stream_stats_started', 'event_stream_stats_retrieved', 'event_stream_stats_retrieval_failed',
            'event_publish_started', 'event_published', 'event_publish_failed',
            'event_replay_started', 'event_replay_completed', 'event_replay_failed',
            'event_clear_started', 'event_clear_completed', 'event_clear_failed'
        }
        ...
        with patch('services.orchestrator.presentation.api.infrastructure.routes.container') as mock_container:
            ...
            key_events_logged = logged_events.intersection({
                'saga_creation_started', 'saga_created',
                'dlq_stats_retrieval_started', 'dlq_stats_retrieved',
                'event_publish_started', 'event_published'
            })
