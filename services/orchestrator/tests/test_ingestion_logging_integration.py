...
class TestOrchestratorIngestionLoggingIntegration:
    ...
    async def test_ingestion_workflow_creation_logging_success(self, client, mock_logger_client):
        ...
        mock_result = {
            ...
            "status": "started",
            "message": "Ingestion workflow initiated"
        }
        ...
        with patch('services.orchestrator.presentation.api.ingestion.routes.container') as mock_container:
            ...
            workflow_started = next((call for call in business_calls
                                   if call[0][0] == 'ingestion_workflow_started'), None)
            workflow_created = next((call for call in business_calls
                                   if call[0][0] == 'ingestion_workflow_created'), None)
    ...
    async def test_ingestion_status_retrieval_logging_success(self, client, mock_logger_client):
        ...
        with patch('services.orchestrator.presentation.api.ingestion.routes.container') as mock_container:
            ...
            business_calls = mock_logger_client.log_business_event.call_args_list
            status_retrieved = next((call for call in business_calls
                                   if call[0][0] == 'ingestion_status_retrieved'), None)
    ...
    async def test_ingestion_status_retrieval_logging_not_found(self, client, mock_logger_client):
        ...
        with patch('services.orchestrator.presentation.api.ingestion.routes.container') as mock_container:
            ...
            # Verify logging calls
            business_calls = mock_logger_client.log_business_event.call_args_list
            ingestion_not_found = next((call for call in business_calls
                                      if call[0][0] == 'ingestion_not_found'), None)
    ...
    async def test_ingestion_listing_logging_with_filters(self, client, mock_logger_client):
        ...
        with patch('services.orchestrator.presentation.api.ingestion.routes.container') as mock_container:
            ...
            listing_started = next((call for call in business_calls
                                  if call[0][0] == 'ingestion_listing_started'), None)
            listing_completed = next((call for call in business_calls
                                    if call[0][0] == 'ingestion_listing_completed'), None)
    ...
    async def test_document_metadata_retrieval_logging_placeholder(self, client, mock_logger_client):
        ...
        business_calls = mock_logger_client.log_business_event.call_args_list
        metadata_started = next((call for call in business_calls
                               if call[0][0] == 'document_metadata_retrieval_started'), None)
        not_implemented = next((call for call in business_calls
                              if call[0][0] == 'document_metadata_not_implemented'), None)
    ...
    async def test_ingestion_sources_listing_logging_success(self, client, mock_logger_client):
        ...
        business_calls = mock_logger_client.log_business_event.call_args_list
        sources_started = next((call for call in business_calls
                              if call[0][0] == 'ingestion_sources_listing_started'), None)
        sources_listed = next((call for call in business_calls
                             if call[0][0] == 'ingestion_sources_listed'), None)
    ...
    async def test_ingestion_cancellation_logging_placeholder(self, client, mock_logger_client):
        ...
        business_calls = mock_logger_client.log_business_event.call_args_list
        cancellation_started = next((call for call in business_calls
                                   if call[0][0] == 'ingestion_cancellation_started'), None)
        not_implemented = next((call for call in business_calls
                              if call[0][0] == 'ingestion_cancellation_not_implemented'), None)
    ...
    async def test_ingestion_workflow_creation_logging_failure(self, client, mock_logger_client):
        ...
        with patch('services.orchestrator.presentation.api.ingestion.routes.container') as mock_container:
            ...
            workflow_failed = next((call for call in business_calls
                                  if call[0][0] == 'ingestion_workflow_creation_failed'), None)
    ...
    async def test_request_ids_generated_uniquely(self, client, mock_logger_client):
        ...
        with patch('services.orchestrator.presentation.api.ingestion.routes.container') as mock_container:
            ...
            client.post("/api/v1/ingestion/ingest", json={
                "source_url": "https://github.com/test/repo", "source_type": "github"
            })
        ...
        for request_id in request_ids:
            assert any(prefix in request_id for prefix in [
                'ingestion_start_', 'ingestion_status_', 'ingestions_list_',
                'document_metadata_', 'ingestion_sources_', 'ingestion_cancel_'
            ])
    ...
    async def test_business_events_comprehensive_coverage(self, client, mock_logger_client):
        ...
        expected_events = {
            ...
            'ingestion_workflow_started', 'ingestion_workflow_created', 'ingestion_workflow_creation_failed',

            # Ingestion status events
            'ingestion_status_retrieval_started', 'ingestion_status_retrieved', 'ingestion_not_found',
            'ingestion_status_retrieval_failed',

            # Ingestion listing events
            'ingestion_listing_started', 'ingestion_listing_completed', 'ingestion_listing_failed',

            # Document metadata events
            'document_metadata_retrieval_started', 'document_metadata_not_implemented', 'document_metadata_retrieval_failed',

            # Ingestion sources events
            'ingestion_sources_listing_started', 'ingestion_sources_listed', 'ingestion_sources_listing_failed',

            # Ingestion control events
            'ingestion_cancellation_started', 'ingestion_cancellation_not_implemented', 'ingestion_cancellation_failed'
        }
        ...
        with patch('services.orchestrator.presentation.api.ingestion.routes.container') as mock_container:
            ...
            key_events_logged = logged_events.intersection({
                'ingestion_workflow_started', 'ingestion_workflow_created',
                'ingestion_status_retrieval_started', 'ingestion_status_retrieved',
                'ingestion_listing_started', 'ingestion_listing_completed',
                'document_metadata_retrieval_started', 'document_metadata_not_implemented',
                'ingestion_sources_listing_started', 'ingestion_sources_listed',
                'ingestion_cancellation_started', 'ingestion_cancellation_not_implemented'
            })
