...
async def run_langgraph_workflow(workflow_type: str, req: LangGraphWorkflowRequest, request: Request):
    ...
    try:
        ...
        workflow_req = WorkflowRunRequest(
            workflow_type=workflow_type, parameters=req.parameters, user_id=req.user_id, correlation_id=request_id
        )
        ...
        if logger:
            ...
            await logger.log_performance_metric(
                ...
                response_time,
                {"request_id": request_id, "workflow_type": workflow_type, "success": True},
            )
