...
async def registry_sync_peers(request: Request):
    ...
    try:
        peers = [
            p.strip()
            for p in (get_config_value("ORCHESTRATOR_PEERS", "", section="orchestrator").split(","))
            if p.strip()
        ]
        ...
        if logger:
            ...
            await logger.log_info(
                "Starting registry synchronization with peers",
                {"request_id": request_id, "peer_count": len(peers), "services_count": len(registry)},
            )
                ...
                try:
                    ...
                    if logger:
                        await logger.log_warning(
                            "Registry sync failed for service",
                            {"request_id": request_id, "peer": peer, "service": service_name, "error": str(e)},
                        )

            # Log per-peer results
            if logger and peer_sent > 0:
                await logger.log_info(
                    "Registry sync completed for peer",
                    {
                        "request_id": request_id,
                        "peer": peer,
