...
class HealthCheckService:
    ...
    async def _perform_custom_check(
        ...
        try:
            result = await asyncio.wait_for(check_function(), timeout=timeout_seconds)
            ...
            if isinstance(result, HealthCheckResult):
                ...
                return (
                    HealthCheckResult.success("Custom check passed")
                    if result
                    else HealthCheckResult.failure("Custom check failed")
                )
            else:
