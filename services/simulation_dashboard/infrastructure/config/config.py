...
class SimulationServiceConfig(BaseModel):
    ...
    base_url: str = Field(
        default="", description="Full base URL (auto-generated if empty)"
    )
...
class WebSocketConfig(BaseModel):
    ...
    reconnect_delay: float = Field(
        default=2.0, description="Delay between reconnection attempts"
    )
    heartbeat_interval: float = Field(
        default=30.0, description="Heartbeat interval in seconds"
    )
    message_timeout: float = Field(
        default=10.0, description="Message timeout in seconds"
    )
...
class DashboardConfig(BaseModel):
    ...
    title: str = Field(
        default="Project Simulation Dashboard", description="Dashboard title"
    )
    theme: str = Field(default="light", description="Dashboard theme (light/dark)")
    refresh_interval: int = Field(
        default=5000, description="Auto-refresh interval in milliseconds"
    )
    max_simulations_display: int = Field(
        default=50, description="Maximum simulations to display"
    )
...
class DashboardSettings(BaseSettings):
    ...
    service_name: str = Field(
        default="simulation-dashboard", description="Service name"
    )
    ...
    simulation_service: SimulationServiceConfig = Field(
        default_factory=SimulationServiceConfig
    )
    ...
    analysis_service_url: Optional[str] = Field(
        default=None, description="Analysis service URL"
    )
    health_service_url: Optional[str] = Field(
        default=None, description="Health service URL"
    )
