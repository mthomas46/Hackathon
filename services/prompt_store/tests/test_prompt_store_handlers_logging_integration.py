...
class TestPromptStoreHandlersLoggingIntegration:
    ...
    async def setup_logger_client(self, mock_logger_client):
        ...
        with patch('services.prompt_store.domain.ab_testing.handlers.get_logger_client') as mock_ab_get_client, \
             patch('services.prompt_store.domain.analytics.handlers.get_logger_client') as mock_analytics_get_client:
    ...
    async def test_ab_test_creation_logging_success(self, client, mock_logger_client):
        ...
        with patch('services.prompt_store.domain.ab_testing.handlers.ABTestService') as mock_service_class:
            ...
            creation_started = next((call for call in business_calls
                                   if call[0][0] == 'ab_test_creation_started'), None)
            test_created = next((call for call in business_calls
                               if call[0][0] == 'ab_test_created'), None)
    ...
    async def test_ab_test_prompt_selection_logging_success(self, client, mock_logger_client):
        ...
        with patch('services.prompt_store.domain.ab_testing.handlers.ABTestService') as mock_service_class:
            ...
            business_calls = mock_logger_client.log_business_event.call_args_list
            selection_started = next((call for call in business_calls
                                    if call[0][0] == 'ab_test_prompt_selection_started'), None)
            prompt_selected = next((call for call in business_calls
                                  if call[0][0] == 'ab_test_prompt_selected'), None)
    ...
    async def test_usage_metrics_recording_logging_success(self, client, mock_logger_client):
        ...
        with patch('services.prompt_store.domain.analytics.handlers.AnalyticsService') as mock_service_class:
            ...
            business_calls = mock_logger_client.log_business_event.call_args_list
            recording_started = next((call for call in business_calls
                                    if call[0][0] == 'usage_metrics_recording_started'), None)
            metrics_recorded = next((call for call in business_calls
                                   if call[0][0] == 'usage_metrics_recorded'), None)
    ...
    async def test_analytics_dashboard_logging_success(self, client, mock_logger_client):
        ...
        with patch('services.prompt_store.domain.analytics.handlers.AnalyticsService') as mock_service_class:
            ...
            retrieval_started = next((call for call in business_calls
                                    if call[0][0] == 'analytics_dashboard_retrieval_started'), None)
            dashboard_retrieved = next((call for call in business_calls
                                      if call[0][0] == 'analytics_dashboard_retrieved'), None)
    ...
    async def test_ab_test_creation_logging_validation_failure(self, client, mock_logger_client):
        ...
        with patch('services.prompt_store.domain.ab_testing.handlers.ABTestService') as mock_service_class:
            ...
            validation_failed = next((call for call in business_calls
                                    if call[0][0] == 'ab_test_creation_validation_failed'), None)
    ...
    async def test_ab_test_prompt_selection_logging_test_not_found(self, client, mock_logger_client):
        ...
        with patch('services.prompt_store.domain.ab_testing.handlers.ABTestService') as mock_service_class:
            ...
            business_calls = mock_logger_client.log_business_event.call_args_list
            test_not_found = next((call for call in business_calls
                                 if call[0][0] == 'ab_test_not_found'), None)
    ...
    async def test_usage_metrics_recording_logging_failure(self, client, mock_logger_client):
        ...
        with patch('services.prompt_store.domain.analytics.handlers.AnalyticsService') as mock_service_class:
            ...
            business_calls = mock_logger_client.log_business_event.call_args_list
            recording_failed = next((call for call in business_calls
                                   if call[0][0] == 'usage_metrics_recording_failed'), None)
    ...
    async def test_analytics_dashboard_logging_failure(self, client, mock_logger_client):
        ...
        with patch('services.prompt_store.domain.analytics.handlers.AnalyticsService') as mock_service_class:
            ...
            assert mock_logger_client.log_error.call_count == 1
            business_calls = mock_logger_client.log_business_event.call_args_list
            retrieval_failed = next((call for call in business_calls
                                   if call[0][0] == 'analytics_dashboard_retrieval_failed'), None)
    ...
    async def test_request_ids_generated_uniquely(self, mock_logger_client):
        ...
        with patch('services.prompt_store.domain.ab_testing.handlers.ABTestService') as mock_ab_service_class, \
             patch('services.prompt_store.domain.analytics.handlers.AnalyticsService') as mock_analytics_service_class:
            ...
            await ab_handler.handle_create_ab_test({
                "name": "Test", "prompt_variants": [], "target_metric": "test"
            })
        ...
        for request_id in request_ids:
            assert any(prefix in request_id for prefix in [
                'ab_test_create_', 'ab_test_select_', 'usage_metrics_record_', 'analytics_dashboard_'
            ])
    ...
    async def test_business_events_comprehensive_coverage(self, mock_logger_client):
        ...
        expected_events = {
            # A/B testing events
            'ab_test_creation_started', 'ab_test_created', 'ab_test_creation_validation_failed',
            'ab_test_creation_failed', 'ab_test_prompt_selection_started', 'ab_test_prompt_selected',
            'ab_test_not_found', 'ab_test_prompt_selection_failed',

            # Analytics events
            'usage_metrics_recording_started', 'usage_metrics_recorded', 'usage_metrics_recording_failed',
            'analytics_dashboard_retrieval_started', 'analytics_dashboard_retrieved',
        ...
        with patch('services.prompt_store.domain.ab_testing.handlers.ABTestService') as mock_ab_service_class, \
            ...
            key_events_logged = logged_events.intersection({
                'ab_test_creation_started', 'ab_test_created',
                'ab_test_prompt_selection_started', 'ab_test_prompt_selected',
                'usage_metrics_recording_started', 'usage_metrics_recorded',
                'analytics_dashboard_retrieval_started', 'analytics_dashboard_retrieved'
            })
