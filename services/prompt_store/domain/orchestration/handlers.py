...
class OrchestrationHandlers(BaseHandler):
    ...
    async def handle_create_conditional_chain(
        ...
        try:
            ...
            return create_success_response(
                message="Conditional chain created successfully", data=chain
            ).model_dump()
        except Exception as e:
            return create_error_response(
                f"Failed to create conditional chain: {str(e)}", "INTERNAL_ERROR"
            ).model_dump()

    async def handle_execute_chain(
        self, chain_id: str, initial_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Execute a conditional chain."""
        try:
            execution = await self.service.execute_conditional_chain(
                chain_id, initial_context
            )
            return create_success_response(
                message="Chain execution completed", data=execution
            ).model_dump()
        except Exception as e:
            return create_error_response(
                f"Failed to execute chain: {str(e)}", "INTERNAL_ERROR"
            ).model_dump()
    ...
    async def handle_create_pipeline(
        ...
        try:
            ...
            return create_success_response(
                message="Pipeline created successfully", data=pipeline
            ).model_dump()
        except Exception as e:
            return create_error_response(
                f"Failed to create pipeline: {str(e)}", "INTERNAL_ERROR"
            ).model_dump()

    async def handle_execute_pipeline(
        ...
        try:
            ...
            return create_success_response(
                message="Pipeline execution completed", data=execution
            ).model_dump()
        except Exception as e:
            return create_error_response(
                f"Failed to execute pipeline: {str(e)}", "INTERNAL_ERROR"
            ).model_dump()
    ...
    async def handle_select_optimal_prompt(
        ...
        try:
            prompt_id = await self.service.select_optimal_prompt(
            ...
            return create_error_response(
                f"Failed to select optimal prompt: {str(e)}", "INTERNAL_ERROR"
            ).model_dump()
    ...
    async def handle_get_recommendations(
        ...
        try:
            recommendations = await self.service.get_prompt_recommendations(
                task_description, context
            )
            return create_success_response(
            ...
            return create_error_response(
                f"Failed to get recommendations: {str(e)}", "INTERNAL_ERROR"
            ).model_dump()
