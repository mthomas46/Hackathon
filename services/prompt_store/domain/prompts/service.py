...
class PromptService(BaseService[Prompt]):
    ...
    def get_generated_documents(self, prompt_id: str) -> List[Dict[str, Any]]:
        """Get all documents generated by this prompt through refinement."""
        try:
            ...
            # doc_store service not available
            return []
        except Exception as e:
            print(f"Error retrieving documents for prompt {prompt_id}: {e}")
            return []

    def get_prompt_document_summary(self, prompt_id: str) -> Dict[str, Any]:
        """Get a summary of documents generated by this prompt."""
        documents = self.get_generated_documents(prompt_id)
