...
class PromptHandlers(BaseHandler):
    ...
    async def handle_get_prompt(self, prompt_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not prompt:
                error_response = create_error_response(
                    f"Prompt '{prompt_id}' not found", "NOT_FOUND"
                )
                return error_response.model_dump()

            response = create_success_response(
                message="Prompt retrieved successfully", data=prompt.to_dict()
            )
            return response.model_dump()
        except ValueError as e:
            error_response = create_error_response(str(e), "VALIDATION_ERROR")
            ...
            error_response = create_error_response(
                f"Failed to retrieve prompt: {str(e)}", "INTERNAL_ERROR"
            )
            return error_response.model_dump()

    async def handle_get_prompt_by_name(
        self, category: str, name: str, **variables
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            error_response = create_error_response(
                f"Failed to get prompt: {str(e)}", "INTERNAL_ERROR"
            )

    async def handle_update_prompt(
        self, prompt_id: str, updates: PromptUpdate
    ) -> Dict[str, Any]:
        """Update a prompt."""
        try:
            updated_prompt = self.service.update_entity(
                prompt_id, updates.model_dump(exclude_unset=True)
            )
            ...
            response = create_success_response(
                message="Prompt updated successfully", data=updated_prompt.to_dict()
            )
            return response.model_dump()
        except ValueError as e:
            error_response = create_error_response(str(e), "VALIDATION_ERROR")
            ...
            error_response = create_error_response(
                f"Failed to update prompt: {str(e)}", "INTERNAL_ERROR"
            )
            return error_response.model_dump()
    ...
    async def handle_delete_prompt(self, prompt_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            error_response = create_error_response(
                f"Failed to delete prompt: {str(e)}", "INTERNAL_ERROR"
            )
    ...
    async def handle_list_prompts(
        ...
        **filters,
        ...
        try:
            ...
            result = self.service.list_entities(
                limit=limit, offset=offset, **search_filters
            )

            return create_success_response(
                message="Prompts retrieved successfully", data=result
            )
        except Exception as e:
            return create_error_response(
                f"Failed to list prompts: {str(e)}", "INTERNAL_ERROR"
            )
    ...
    async def handle_search_prompts(
        ...
        try:
            ...
            return create_success_response(
                message="Prompts searched successfully", data=result
            )
        except Exception as e:
            error_response = create_error_response(
                f"Failed to search prompts: {str(e)}", "INTERNAL_ERROR"
            )
    ...
    async def handle_fork_prompt(
        ...
        """Fork a prompt."""
        try:
            forked_prompt = self.service.fork_prompt(
                prompt_id, new_name, created_by, changes
            )
            return create_success_response(
                message="Prompt forked successfully", data=forked_prompt.to_dict()
            )
        except ValueError as e:
            error_response = create_error_response(str(e), "VALIDATION_ERROR")
        except Exception as e:
            error_response = create_error_response(
                f"Failed to fork prompt: {str(e)}", "INTERNAL_ERROR"
            )
    ...
    async def handle_update_prompt_content(
        ...
        try:
            ...
            error_response = create_error_response(
                f"Failed to update prompt content: {str(e)}", "INTERNAL_ERROR"
            )
    ...
    async def handle_detect_drift(self, prompt_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return create_success_response(
                message="Prompt drift analysis completed", data=drift_info
            )
        except ValueError as e:
            error_response = create_error_response(str(e), "VALIDATION_ERROR")
        except Exception as e:
            error_response = create_error_response(
                f"Failed to detect drift: {str(e)}", "INTERNAL_ERROR"
            )
    ...
    async def handle_get_suggestions(self, prompt_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            error_response = create_error_response(
                f"Failed to get suggestions: {str(e)}", "INTERNAL_ERROR"
            )
    ...
    async def handle_bulk_update_tags(
        ...
        try:
            updated_count = self.service.bulk_update_tags(
            ...
            error_response = create_error_response(
                f"Failed to update tags: {str(e)}", "INTERNAL_ERROR"
            )
