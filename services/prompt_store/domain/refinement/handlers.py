...
class PromptRefinementHandlers:
    ...
    async def handle_refine_prompt(
        ...
        try:
            ...
            return create_success_response(
                message="Prompt refinement started successfully", data=result
            )
        except ValueError as e:
            return create_error_response(str(e), "VALIDATION_ERROR")
        except Exception as e:
            return create_error_response(
                f"Failed to start prompt refinement: {str(e)}", "INTERNAL_ERROR"
            )
    ...
    async def handle_get_refinement_status(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return create_success_response(
                message="Refinement status retrieved successfully", data=status
            )
        except ValueError as e:
            return create_error_response(str(e), "VALIDATION_ERROR")
        except Exception as e:
            return create_error_response(
                f"Failed to get refinement status: {str(e)}", "INTERNAL_ERROR"
            )
    ...
    async def handle_compare_prompt_versions(
        ...
        try:
            ...
            return create_success_response(
                message="Prompt versions compared successfully", data=comparison
            )
        except ValueError as e:
            return create_error_response(str(e), "VALIDATION_ERROR")
        except Exception as e:
            return create_error_response(
                f"Failed to compare prompt versions: {str(e)}", "INTERNAL_ERROR"
            )

    async def handle_compare_refinement_documents(
        self, session_a: str, session_b: str
    ) -> Dict[str, Any]:
        """Compare documents from different refinement sessions."""
        try:
            comparison = await self.service.compare_refinement_documents(
                session_a, session_b
            )
            return create_success_response(
                message="Refinement documents compared successfully", data=comparison
            )
        except ValueError as e:
            return create_error_response(str(e), "VALIDATION_ERROR")
        except Exception as e:
            return create_error_response(
                f"Failed to compare refinement documents: {str(e)}", "INTERNAL_ERROR"
            )
    ...
    async def handle_replace_prompt_with_refined(
        ...
        try:
            ...
            return create_success_response(
                message="Prompt replaced with refined version successfully", data=result
            )
        except ValueError as e:
            return create_error_response(str(e), "VALIDATION_ERROR")
        except Exception as e:
            return create_error_response(
                f"Failed to replace prompt: {str(e)}", "INTERNAL_ERROR"
            )
    ...
    async def handle_get_refinement_history(self, prompt_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return create_success_response(
                message="Refinement history retrieved successfully", data=history
            )
        except ValueError as e:
            return create_error_response(str(e), "VALIDATION_ERROR")
        except Exception as e:
            return create_error_response(
                f"Failed to get refinement history: {str(e)}", "INTERNAL_ERROR"
            )

    async def handle_get_version_refinement_details(
        self, prompt_id: str, version: int
    ) -> Dict[str, Any]:
        """Get detailed refinement information for a specific version."""
        try:
            details = await self.service.get_version_refinement_details(
                prompt_id, version
            )
            return create_success_response(
            ...
            return create_error_response(
                f"Failed to get version refinement details: {str(e)}", "INTERNAL_ERROR"
            )

    async def handle_list_active_refinements(
        ...
        try:
            ...
            return create_success_response(
                ...
                data={"active_sessions": [], "total_active": 0},
            )
        except Exception as e:
            return create_error_response(
                f"Failed to list active refinements: {str(e)}", "INTERNAL_ERROR"
            )
