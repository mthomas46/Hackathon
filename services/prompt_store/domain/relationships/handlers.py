...
class RelationshipsHandlers:
    ...
    def handle_get_relationships(
        ...
        try:
            result = self.relationships_service.get_relationships_for_prompt(
                prompt_id, direction
            )

            response = create_success_response(
                data=result, message=f"Retrieved relationships for prompt {prompt_id}"
            )
            return response.model_dump()
    ...
    def handle_update_relationship_strength(
        ...
        try:
            ...
            response = create_success_response(
                data=result, message=f"Relationship strength updated to {new_strength}"
            )
            return response.model_dump()
    ...
    def handle_delete_relationship(
        ...
        try:
            result = self.relationships_service.delete_relationship(
                relationship_id, user_id
            )

            response = create_success_response(
                data=result, message="Relationship deleted successfully"
            )
            return response.model_dump()
    ...
    def handle_get_relationship_graph(
        self, prompt_id: str, depth: int = 2
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to retrieve relationship graph: {str(e)}",
            )
    ...
    def handle_get_relationship_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = create_success_response(
                data=result, message="Retrieved relationship statistics"
            )
            return response.model_dump()
    ...
    def handle_find_related_prompts(
        ...
        try:
            ...
            response = create_success_response(
                data=result, message=f"Found {result['total_related']} related prompts"
            )
            return response.model_dump()
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to find related prompts: {str(e)}"
            )
    ...
    def handle_validate_relationship(
        ...
        try:
            ...
            response = create_success_response(
                data=result, message="Relationship validation completed"
            )
            return response.model_dump()

        except Exception as e:
            raise HTTPException(
                status_code=500, detail=f"Failed to validate relationship: {str(e)}"
            )
