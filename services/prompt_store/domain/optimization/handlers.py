...
class OptimizationHandlers(BaseHandler):
    ...
    async def handle_create_ab_test(
        self, prompt_a_id: str, prompt_b_id: str, traffic_percentage: float = 50.0
    ) -> Dict[str, Any]:
        """Create a new A/B test."""
        try:
            test = await self.service.create_ab_test(
                prompt_a_id, prompt_b_id, traffic_percentage
            )
            return create_success_response(
                message="A/B test created successfully", data=test.get_results()
            ).model_dump()
    ...
    async def handle_get_prompt_assignment(
        ...
        try:
            ...
            if prompt_id:
                ...
                return create_error_response(
                    "Test not found or not running", "NOT_FOUND"
                ).model_dump()
        except Exception as e:
            return create_error_response(
                f"Failed to get prompt assignment: {str(e)}", "INTERNAL_ERROR"
            ).model_dump()
    ...
    async def handle_get_test_results(self, test_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if results:
                return create_success_response(
                    message="Test results retrieved successfully", data=results
                ).model_dump()
            else:
    ...
    async def handle_run_optimization(
        self, prompt_id: str, base_version: int
    ) -> Dict[str, Any]:
        """Run automated optimization cycle."""
        try:
            cycle = await self.service.run_automated_optimization(
                prompt_id, base_version
            )
            return create_success_response(
                message="Optimization cycle started successfully", data=cycle
            ).model_dump()
        except Exception as e:
            return create_error_response(
                f"Failed to start optimization: {str(e)}", "INTERNAL_ERROR"
            ).model_dump()

    async def handle_generate_variations(
        self, prompt_content: str, count: int = 3
    ) -> Dict[str, Any]:
        """Generate prompt variations."""
        try:
            variations = await self.service.generate_prompt_variations(
            ...
            return create_error_response(
                f"Failed to generate variations: {str(e)}", "INTERNAL_ERROR"
            ).model_dump()
