...
class ABTestHandlers(BaseHandler):
    ...
    async def handle_create_ab_test(self, test_data: ABTestCreate) -> Dict[str, Any]:
        ...
        try:
            ...
            if logger:
                await logger.log_business_event("ab_test_creation_started", {
                ...
                await logger.log_info("Initiating A/B test creation", {
                    "request_id": request_id,
                    "test_name": test_data.name,
            ...
            if logger:
                await logger.log_business_event("ab_test_created", {
                    "request_id": request_id,
                    "test_id": test_id,
                    "operation": "ab_testing_workflow",
            ...
            if logger:
                await logger.log_business_event("ab_test_creation_validation_failed", {
                    "request_id": request_id,
                    "operation": "ab_testing_workflow",
                    "test_name": test_data.name,
            ...
            if logger:
                ...
                await logger.log_business_event("ab_test_creation_failed", {
                    "request_id": request_id,
                    "operation": "ab_testing_workflow",
                    "error_type": type(e).__name__,
                    "error_message": str(e),
    ...
    async def handle_select_prompt_for_test(self, test_id: str, user_id: Optional[str] = None,
        ...
        try:
            ...
            if logger:
                await logger.log_business_event("ab_test_prompt_selection_started", {
                    "request_id": request_id,
                    "test_id": test_id,
                ...
                await logger.log_info("Selecting prompt variant for A/B testing", {
                    "request_id": request_id,
                    "test_id": test_id,
            ...
            if not result:
                ...
                if logger:
                    await logger.log_business_event("ab_test_not_found", {
                        "request_id": request_id,
                        "test_id": test_id,
                        "operation": "ab_testing_experimentation",
                        "response_time_seconds": response_time,
            ...
            if logger:
                await logger.log_business_event("ab_test_prompt_selected", {
                    "request_id": request_id,
                    "test_id": test_id,
            ...
            if logger:
                ...
                await logger.log_business_event("ab_test_prompt_selection_failed", {
                    "request_id": request_id,
                    "operation": "ab_testing_experimentation",
                    "test_id": test_id,
                    "error_type": type(e).__name__,
