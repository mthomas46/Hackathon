...
class TestArchitectureDigitizerLoggingIntegration:
    ...
    async def test_architecture_normalization_successful_logging(self, client, mock_logger_client):
        ...
        with patch('main.get_normalizer') as mock_get_normalizer, \
             patch('main.record_architecture_digitizer_request') as mock_record_request, \
             patch('main.fire_and_forget') as mock_fire_and_forget:
            ...
            completion_call = next(call for call in business_calls if call[0][0] == 'architecture_normalization_completed')
    ...
    async def test_architecture_normalization_failure_logging(self, client, mock_logger_client):
        ...
        with patch('main.get_normalizer') as mock_get_normalizer, \
             patch('main.record_architecture_digitizer_request') as mock_record_request, \
             patch('main.record_architecture_digitizer_api_failure') as mock_record_failure:
    ...
    async def test_different_diagram_systems_logging(self, client, mock_logger_client):
        ...
        for system in systems:
            ...
            with patch('main.get_normalizer') as mock_get_normalizer, \
                ...
                request_data = {
                    "system": system,
                    "board_id": f"{system}_board_123",
                    "token": f"{system}_token"
                }
