...
def build_discovery_context(operation: str, **additional) -> Dict[str, Any]:
    ...
    """
    context = {"service": ServiceNames.DISCOVERY_AGENT}
...
def fetch_openapi_spec(openapi_url: str, timeout: int = _DEFAULT_TIMEOUT) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ServiceException(
            ...
            details={"openapi_url": openapi_url, "error": str(e)},
        )
...
def create_discovery_response(
    endpoints: List[Dict[str, Any]], metadata: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
...
async def _async_register_with_orchestrator(
