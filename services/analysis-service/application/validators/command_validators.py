...
class CreateDocumentCommandValidator(BaseValidator):
    ...
    async def validate(self, command: CreateDocumentCommand) -> ValidationResult:
        ...
        if not command.title or not isinstance(command.title, str):
            errors.append(
                self.create_error("Document title is required and must be a string", "INVALID_TITLE", "title")
            )
            ...
            errors.append(self.create_error("Document title cannot exceed 200 characters", "TITLE_TOO_LONG", "title"))

        # Validate content
        if not command.content or not isinstance(command.content, str):
            errors.append(
                self.create_error("Document content is required and must be a string", "INVALID_CONTENT", "content")
            )
        ...
        if command.author and not isinstance(command.author, str):
            ...
            errors.append(
                self.create_error("Document author cannot exceed 100 characters", "AUTHOR_TOO_LONG", "author")
            )
            ...
            if not isinstance(command.tags, list):
                ...
                for i, tag in enumerate(command.tags):
                    if not isinstance(tag, str):
                        errors.append(
                            self.create_error(f"Tag at index {i} must be a string", "INVALID_TAG_TYPE", f"tags[{i}]")
                        )
                    elif len(tag) > 50:
                        errors.append(
                            self.create_error(
                                f"Tag '{tag}' exceeds maximum length of 50 characters", "TAG_TOO_LONG", f"tags[{i}]"
                            )
                        )
        ...
        if command.metadata:
            if not isinstance(command.metadata, dict):
                errors.append(
                    self.create_error("Document metadata must be a dictionary", "INVALID_METADATA", "metadata")
                )
            elif len(str(command.metadata)) > 10000:  # Rough size check
                errors.append(self.create_error("Document metadata is too large", "METADATA_TOO_LARGE", "metadata"))
    ...
    async def validate(self, command: UpdateDocumentCommand) -> ValidationResult:
        ...
        if hasattr(command, "title") and command.title is not None:
            if not isinstance(command.title, str):
                ...
                errors.append(
                    self.create_error("Document title cannot exceed 200 characters", "TITLE_TOO_LONG", "title")
                )

        # Validate content if provided
        if hasattr(command, "content") and command.content is not None:
            if not isinstance(command.content, str):
                errors.append(self.create_error("Document content must be a string", "INVALID_CONTENT", "content"))
            elif len(command.content.strip()) == 0:
                errors.append(self.create_error("Document content cannot be empty", "EMPTY_CONTENT", "content"))
...
class PerformAnalysisCommandValidator(BaseValidator):
    ...
    async def validate(self, command: PerformAnalysisCommand) -> ValidationResult:
        ...
        if not command.analysis_type or not isinstance(command.analysis_type, str):
            errors.append(
                self.create_error(
                    "Analysis type is required and must be a string", "INVALID_ANALYSIS_TYPE", "analysis_type"
                )
            )
            ...
            errors.append(
                self.create_error(
                    f"Invalid analysis type: {command.analysis_type}", "UNSUPPORTED_ANALYSIS_TYPE", "analysis_type"
                )
            )
        ...
        if command.configuration:
            if not isinstance(command.configuration, dict):
                errors.append(
                    self.create_error(
                        "Analysis configuration must be a dictionary", "INVALID_CONFIGURATION", "configuration"
                    )
                )
            else:
                ...
                if "timeout_seconds" in command.configuration:
                    ...
                    if not isinstance(timeout, (int, float)):
                        errors.append(
                            self.create_error(
                                "Timeout must be a number", "INVALID_TIMEOUT", "configuration.timeout_seconds"
                            )
                        )
                ...
                if "priority" in command.configuration:
                    ...
                    if priority not in ["low", "normal", "high", "critical"]:
                        errors.append(
                            self.create_error(
                                f"Invalid priority: {priority}", "INVALID_PRIORITY", "configuration.priority"
                            )
                        )
        ...
        if hasattr(command, "timeout_seconds") and command.timeout_seconds is not None:
            if not isinstance(command.timeout_seconds, (int, float)):
                ...
                errors.append(
                    self.create_error(
                        "Timeout must be between 10 and 3600 seconds", "INVALID_TIMEOUT_RANGE", "timeout_seconds"
                    )
                )
...
class CreateFindingCommandValidator(BaseValidator):
    ...
    async def validate(self, command: CreateFindingCommand) -> ValidationResult:
        ...
        if not command.severity or not isinstance(command.severity, str):
            errors.append(
                self.create_error("Finding severity is required and must be a string", "INVALID_SEVERITY", "severity")
            )
        elif command.severity not in ["critical", "high", "medium", "low", "info"]:
            errors.append(
                self.create_error(f"Invalid severity: {command.severity}", "UNSUPPORTED_SEVERITY", "severity")
            )

        # Validate category
        if not command.category or not isinstance(command.category, str):
            errors.append(
                self.create_error("Finding category is required and must be a string", "INVALID_CATEGORY", "category")
            )
        elif len(command.category) > 50:
            errors.append(
                self.create_error("Finding category cannot exceed 50 characters", "CATEGORY_TOO_LONG", "category")
            )
        ...
        if not command.description or not isinstance(command.description, str):
            errors.append(
                self.create_error(
                    "Finding description is required and must be a string", "INVALID_DESCRIPTION", "description"
                )
            )
        elif len(command.description) > 1000:
            errors.append(
                self.create_error(
                    "Finding description cannot exceed 1000 characters", "DESCRIPTION_TOO_LONG", "description"
                )
            )
        ...
        if command.confidence is not None:
            if not isinstance(command.confidence, (int, float)):
                errors.append(
                    self.create_error("Finding confidence must be a number", "INVALID_CONFIDENCE", "confidence")
                )
            elif command.confidence < 0.0 or command.confidence > 1.0:
                errors.append(
                    self.create_error(
                        "Finding confidence must be between 0.0 and 1.0", "CONFIDENCE_OUT_OF_RANGE", "confidence"
                    )
                )
        ...
        if command.location:
            if not isinstance(command.location, dict):
                errors.append(
                    self.create_error("Finding location must be a dictionary", "INVALID_LOCATION", "location")
                )
        ...
        if command.suggestion and len(command.suggestion) > 500:
            errors.append(
                self.create_error(
                    "Finding suggestion cannot exceed 500 characters", "SUGGESTION_TOO_LONG", "suggestion"
                )
            )
