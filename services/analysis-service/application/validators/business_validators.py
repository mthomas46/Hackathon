...
class DocumentBusinessValidator(BaseValidator):
    ...
    async def validate(self, document: Document) -> ValidationResult:
        ...
        if content_size > 10 * 1024 * 1024:  # 10MB
            errors.append(
                self.create_error("Document content exceeds maximum size limit of 10MB", "DOCUMENT_TOO_LARGE")
            )
        ...
        if word_count > 100000:  # 100K words
            warnings.append(
                self.create_warning(
                    f"Document has {word_count} words, which may impact analysis performance", "LARGE_DOCUMENT"
                )
            )
        ...
        if self.document_repository:
            try:
                ...
                if existing_docs:
                    warnings.append(
                        self.create_warning(
                            f"Found {len(existing_docs)} document(s) with similar title", "POTENTIAL_DUPLICATE_TITLE"
                        )
                    )
            except Exception:
        ...
        if document.metadata.created_at > document.metadata.updated_at:
            errors.append(self.create_error("Document creation date cannot be after update date", "INVALID_DATE_ORDER"))

        # Check for stale content
        days_since_update = (datetime.now(timezone.utc) - document.metadata.updated_at).days
        if days_since_update > 365:  # 1 year
            warnings.append(
                self.create_warning(f"Document has not been updated for {days_since_update} days", "STALE_CONTENT")
            )
        ...
        if len(tags) > 20:
            warnings.append(
                self.create_warning(f"Document has {len(tags)} tags, consider consolidating", "TOO_MANY_TAGS")
            )
        ...
        for tag in tags:
            if tag.lower() in meaningless_tags:
                warnings.append(
                    self.create_warning(f"Tag '{tag}' may not provide meaningful categorization", "MEANINGLESS_TAG")
                )
    ...
    async def validate(self, analysis: Analysis) -> ValidationResult:
        ...
        if self.document_repository:
            try:
                ...
                if not document:
                    errors.append(
                        self.create_error(f"Document {analysis.document_id.value} does not exist", "DOCUMENT_NOT_FOUND")
                    )
                else:
                    ...
                    if document.word_count < 10:
                        warnings.append(
                            self.create_warning(
                                "Document has very few words, analysis may not be meaningful", "INSUFFICIENT_CONTENT"
                            )
                        )
            except Exception:
                errors.append(self.create_error("Could not verify document existence", "DOCUMENT_VERIFICATION_FAILED"))
        ...
        if analysis.analysis_type not in supported_types:
            errors.append(
                self.create_error(
                    f"Analysis type '{analysis.analysis_type}' is not supported", "UNSUPPORTED_ANALYSIS_TYPE"
                )
            )
        ...
        if analysis.status.value == "completed" and not analysis.result:
            warnings.append(
                self.create_warning("Analysis is marked as completed but has no results", "MISSING_RESULTS")
            )

        if analysis.status.value == "failed" and not analysis.error_message:
            warnings.append(
                self.create_warning("Analysis is marked as failed but has no error message", "MISSING_ERROR_MESSAGE")
            )
        ...
        if self.analysis_repository and analysis.status.value == "pending":
            try:
                ...
                recent_similar = [
                    a for a in recent_analyses if a.created_at.timestamp() > recent_cutoff and a.id != analysis.id
                ]
        ...
        if analysis.started_at and analysis.completed_at:
            ...
            if execution_time > 3600:  # 1 hour
                warnings.append(
                    self.create_warning(
                        f"Analysis took {execution_time:.0f} seconds, which is unusually long", "LONG_EXECUTION_TIME"
                    )
                )
            elif execution_time < 1:  # Less than 1 second
                warnings.append(
                    self.create_warning(
                        "Analysis completed very quickly, results may not be comprehensive", "FAST_EXECUTION"
                    )
                )
    ...
    async def validate(self, finding: Finding) -> ValidationResult:
        ...
        if self.document_repository:
            try:
                ...
                if not document:
                    errors.append(
                        self.create_error(f"Document {finding.document_id.value} does not exist", "DOCUMENT_NOT_FOUND")
                    )
            except Exception:
                errors.append(self.create_error("Could not verify document existence", "DOCUMENT_VERIFICATION_FAILED"))
        ...
        if self.analysis_repository:
            try:
                ...
                if not analysis:
                    errors.append(
                        self.create_error(f"Analysis {finding.analysis_id.value} does not exist", "ANALYSIS_NOT_FOUND")
                    )
                elif analysis.status.value != "completed":
                    warnings.append(
                        self.create_warning(
                            f"Analysis {finding.analysis_id.value} is not completed", "ANALYSIS_NOT_COMPLETED"
                        )
                    )
            except Exception:
                errors.append(self.create_error("Could not verify analysis existence", "ANALYSIS_VERIFICATION_FAILED"))
        ...
        if self.finding_repository:
            try:
                ...
                if similar_findings:
                    warnings.append(
                        self.create_warning(
                            f"Found {len(similar_findings)} similar finding(s)", "POTENTIAL_DUPLICATE_FINDING"
                        )
                    )
            except Exception:
                # Repository not available or error, skip this check
        ...
        if finding_age_days > 30 and finding.status == "open":
            warnings.append(self.create_warning(f"Finding has been open for {finding_age_days} days", "STALE_FINDING"))
