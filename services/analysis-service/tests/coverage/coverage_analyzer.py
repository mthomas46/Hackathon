...
class CoverageAnalyzer:
    ...
    def __init__(
        self,
        source_dir: str = "services/analysis-service",
        test_dir: str = "services/analysis-service/tests",
    ):
    ...
    def _generate_recommendations(
        self, layer_analysis: Dict[str, Dict[str, Any]], critical_uncovered: Dict[str, List[str]]
    ) -> List[str]:
        ...
        if critical_uncovered:
            recommendations.append(f"Add tests for {len(critical_uncovered)} files with critical uncovered code")

            # Identify most critical files
            critical_files = list(critical_uncovered.keys())[:5]  # Top 5
            if critical_files:
                recommendations.append(f"Priority files to test: {', '.join(critical_files)}")
...
class CoverageEnforcer:
    ...
    def enforce_coverage(self, coverage_report: Dict[str, Any]) -> bool:
        ...
        for layer_name, layer_data in layer_coverage.items():
            if layer_data["status"] == "fail":
                print(
                    f"❌ Layer {layer_name} coverage requirement not met: "
...
class CoverageTracker:
    ...
    def add_entry(self, coverage_report: Dict[str, Any]):
        ...
        entry = {
            ...
            "overall_coverage": coverage_report["overall_coverage"]["percentage"],
            "layer_coverage": {layer: data["coverage_percentage"] for layer, data in coverage_report["layer_coverage"].items()},
...
def get_coverage_trends():
    ...
    print(f"Overall: {trends['overall_trend']['direction']} " f"({trends['overall_trend']['magnitude']:.2f}%)")
