...
class TestDistributedEndpoints:
    ...
    def test_distributed_endpoints_exist(self, client):
        ...
        for endpoint in endpoints:
            response = (
                client.get(endpoint)
                if endpoint.startswith("/distributed/") and "tasks" not in endpoint
    ...
    async def test_cancel_task_success(self, client):
        ...
        with patch(
            "services.analysis_service.presentation.controllers.distributed_controller.distributed_handlers.handle_cancel_task"
        ) as mock_handler:
            mock_response = {
                "task_id": task_id,
                "status": "cancelled",
    ...
    async def test_start_distributed_processing_success(self, client):
        ...
        with patch(
            "services.analysis_service.presentation.controllers.distributed_controller.distributed_handlers.handle_start_processing"
        ) as mock_handler:
            mock_response = {
                "status": "started",
                "worker_count": 5,
    ...
    async def test_set_load_balancing_strategy_success(self, client):
        ...
        with patch(
            "services.analysis_service.presentation.controllers.distributed_controller.distributed_handlers.handle_set_load_balancing_strategy"
        ) as mock_handler:
            mock_response = {
    ...
    async def test_configure_load_balancing_success(self, client):
        ...
        with patch(
            "services.analysis_service.presentation.controllers.distributed_controller.distributed_handlers.handle_configure_load_balancing"
        ) as mock_handler:
            mock_response = {
                "strategy": "performance_based",
                "worker_count": 6,
    ...
    async def test_get_load_balancing_config_success(self, client):
        ...
        with patch(
            "services.analysis_service.presentation.controllers.distributed_controller.distributed_handlers.handle_get_load_balancing_config"
        ) as mock_handler:
            mock_response = {
                "strategy": "adaptive",
                "worker_count": 7,
