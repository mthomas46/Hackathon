...
class MaintenanceForecastRequest(BaseModel):
    ...
    forecast_period_days: Optional[int] = Field(
        180, ge=1, le=365, description="Forecast period"
    )
...
class MaintenanceForecastResponse(BaseModel):
    ...
    maintenance_forecast: Dict[str, Any] = Field(
        default_factory=dict, description="Maintenance forecast"
    )
    ...
    execution_time_seconds: float = Field(
        ..., description="Time taken to complete forecast"
    )
    error_message: Optional[str] = Field(
        None, description="Error message if forecast failed"
    )
...
class PortfolioMaintenanceForecastResponse(BaseModel):
    ...
    portfolio_forecast: Dict[str, Any] = Field(
        default_factory=dict, description="Portfolio forecast"
    )
    maintenance_schedule: Dict[str, Any] = Field(
        default_factory=dict, description="Maintenance schedule"
    )
...
class QualityDegradationDetectionRequest(BaseModel):
    ...
    detection_period_days: Optional[int] = Field(
        60, ge=1, le=365, description="Detection period"
    )
    degradation_threshold: Optional[float] = Field(
        0.1, ge=0.0, le=1.0, description="Degradation threshold"
    )
    options: Optional[Dict[str, Any]] = Field(None, description="Additional options")
...
class QualityDegradationDetectionResponse(BaseModel):
    ...
    degradation_detected: bool = Field(
        ..., description="Whether degradation was detected"
    )
    degradation_score: float = Field(
        ..., ge=0.0, le=1.0, description="Degradation score"
    )
    degradation_trend: Dict[str, Any] = Field(
        default_factory=dict, description="Degradation trend"
    )
    ...
    execution_time_seconds: float = Field(
        ..., description="Time taken to complete detection"
    )
    error_message: Optional[str] = Field(
...
class PortfolioQualityDegradationResponse(BaseModel):
    ...
    portfolio_degradation: Dict[str, Any] = Field(
        default_factory=dict, description="Portfolio degradation"
    )
    degraded_documents: List[str] = Field(
        default_factory=list, description="Degraded documents"
    )
    quality_trends: Dict[str, Any] = Field(
        default_factory=dict, description="Quality trends"
    )
    alerts: List[str] = Field(default_factory=list, description="Degradation alerts")
    execution_time_seconds: float = Field(
        ..., description="Time taken to complete detection"
