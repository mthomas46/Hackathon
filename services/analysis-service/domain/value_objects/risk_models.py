...
class RiskAssessmentRequest(BaseModel):
    ...
    risk_factors: Optional[List[str]] = Field(
        None, description="Risk factors to evaluate"
    )
    assessment_period_days: Optional[int] = Field(
        90, ge=1, le=365, description="Assessment period"
    )
...
class RiskAssessmentResponse(BaseModel):
    ...
    risk_factors: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Risk factor details"
    )
    risk_indicators: List[Dict[str, Any]] = Field(
        default_factory=list, description="Risk indicators"
    )
    mitigation_recommendations: List[Dict[str, Any]] = Field(
...
class PortfolioRiskAssessmentResponse(BaseModel):
    ...
    portfolio_risk_assessment: Dict[str, Any] = Field(
        default_factory=dict, description="Portfolio risk assessment"
    )
    high_risk_documents: List[str] = Field(
        default_factory=list, description="High risk documents"
    )
    risk_mitigation_strategy: Dict[str, Any] = Field(
        default_factory=dict, description="Risk mitigation strategy"
    )
    execution_time_seconds: float = Field(
        ..., description="Time taken to complete assessment"
    )
