...
class AutomatedRemediationResponse(BaseModel):
    ...
    remediation_report: Dict[str, Any] = Field(
        default_factory=dict, description="Remediation report"
    )
    execution_time_seconds: float = Field(
        ..., description="Time taken to complete remediation"
    )
    error_message: Optional[str] = Field(
        None, description="Error message if remediation failed"
    )
...
class RemediationPreviewRequest(BaseModel):
    ...
    issues: List[Dict[str, Any]] = Field(
        ..., description="Issues to preview remediation for"
    )
    remediation_type: Optional[str] = Field("auto", description="Type of remediation")
    options: Optional[Dict[str, Any]] = Field(None, description="Additional options")
...
class RemediationPreviewResponse(BaseModel):
    ...
    impact_assessment: Dict[str, Any] = Field(
        default_factory=dict, description="Impact assessment"
    )
    ...
    execution_time_seconds: float = Field(
        ..., description="Time taken to generate preview"
    )
    error_message: Optional[str] = Field(
        None, description="Error message if preview failed"
    )
