...
class PRConfidenceScorer:
    ...
    def _calculate_code_quality_score(
        self, pr_data: Dict[str, Any], code_analysis_results: Optional[Dict[str, Any]] = None
    ) -> float:
    ...
    def _calculate_security_score(self, pr_data: Dict[str, Any]) -> float:
        ...
        security_changes = [
            f for f in files_changed if any(term in f.lower() for term in ["auth", "security", "encrypt"])
        ]
    ...
    def _identify_risk_factors(
        self, component_scores: Dict[str, float], cross_reference_results: Dict[str, Any]
    ) -> List[str]:
    ...
    def _generate_rationale(
        self, overall_score: float, component_scores: Dict[str, float], risk_factors: List[str]
    ) -> str:
        ...
        rationale_parts.append(
            ", ".join([f"{comp.replace('_', ' ')} ({score:.1%})" for comp, score in top_components[:3]])
        )
        rationale_parts.append(". ")
