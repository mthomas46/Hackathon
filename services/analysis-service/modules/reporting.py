...
from .shared_utils import build_analysis_context, create_analysis_success_response
...
def generate_summary_report(findings: List[Finding]) -> Dict[str, Any]:
    ...
    return create_analysis_success_response(
        "summary report generation", result, **context
    )
...
def generate_trends_report(
    ...
    context = build_analysis_context(
        "trends_report", total_findings=len(findings), time_window=time_window
    )
    result = _generate_trends_report(findings, time_window)
    return create_analysis_success_response(
        "trends report generation", result, **context
    )
