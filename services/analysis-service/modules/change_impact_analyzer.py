...
class ChangeImpactAnalyzer:
    ...
    def _calculate_change_impact(
        ...
        related_documents: Dict[str, Dict[str, Any]],
    ) -> Dict[str, Any]:
    ...
    def _assess_stakeholder_impact(
        self, document_features: Dict[str, Any], change_severity: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        impact_level = (
            "critical"
            if stakeholder_impact_score >= 0.8
            else (
                "high"
                if stakeholder_impact_score >= 0.6
                else (
                    "medium"
                    if stakeholder_impact_score >= 0.4
                    else "low" if stakeholder_impact_score >= 0.2 else "minimal"
                )
            )
        )
...
async def analyze_portfolio_change_impact(
    ...
    return await change_impact_analyzer.analyze_portfolio_change_impact(
        changes=changes, document_portfolio=document_portfolio
    )
