...
def _get_quality_indicator(score: float) -> str:
    """Get quality indicator emoji and description based on score."""
    if score >= config.limits.analysis_confidence_threshold:
        return "🟢 **High Quality** - Documents are well-structured and clear"
    elif score >= 0.6:
        return "🟡 **Medium Quality** - Documents need some improvements"
    else:
        return "🔴 **Low Quality** - Documents require significant attention"
...
def _generate_report_header(report_data: dict) -> list:
...
def _generate_executive_summary(summary: dict) -> list:
    ...
    quality_indicator = _get_quality_indicator(summary["average_quality_score"])
    md_lines.append(f"### Quality Assessment: {quality_indicator}")
    md_lines.append("")
    return md_lines

def _generate_detailed_results(analysis_results: list) -> list:
    ...
    for i, result in enumerate(analysis_results, 1):
        ...
        quality_emoji = _get_quality_emoji(quality_score)
...
def _generate_recommendations(summary: dict) -> list:
...
def generate_analysis_markdown_report(report_data: dict) -> str:
    ...
    # Generate each section using focused helper methods
    md_lines.extend(_generate_report_header(report_data))
    md_lines.extend(_generate_executive_summary(report_data["summary"]))
    md_lines.extend(_generate_detailed_results(report_data["analysis_results"]))
    md_lines.extend(_generate_recommendations(report_data["summary"]))
