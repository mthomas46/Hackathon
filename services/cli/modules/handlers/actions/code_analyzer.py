...
def build_actions(
    ...
    async def analyze_code():
        ...
        console.print(
            "Enter the code to analyze (or leave empty to provide a file path):"
        )
        ...
        if not code_input.strip():
            ...
            try:
                with open(file_path, "r") as f:
                    code_input = f.read()
                console.print(
                    f"✅ Loaded code from {file_path} ({len(code_input)} characters)"
                )
        ...
        include_functions = Prompt.ask("Include functions?", default="yes").lower() in [
            "yes",
            "y",
            "true",
        ]
        include_classes = Prompt.ask("Include classes?", default="yes").lower() in [
            "yes",
            "y",
            "true",
        ]
        ...
        if rx.get("success"):
            ...
            console.print(
                f"[red]❌ Analysis failed: {rx.get('error', 'Unknown error')}[/red]"
            )
        ...
        try:
            ...
            if not language:
                if file_path.endswith(".py"):
                    ...
                    language = Prompt.ask(
                        "Could not auto-detect language. Please specify"
                    )
    ...
    async def batch_analyze():
        ...
        for i in range(count):
            ...
            if analysis_type == "files":
                ...
                try:
                    ...
                    language = Prompt.ask("Language", default="python")
                    items.append(
                        {"name": file_path, "code": code, "language": language}
                    )
                except Exception as e:
        ...
        for result in results:
            ...
            complexity = (
                str(result["complexity"]) if result["complexity"] is not None else "N/A"
            )
