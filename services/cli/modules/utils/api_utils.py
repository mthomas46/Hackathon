...
class APIClient:
    ...
    async def get_json(self, endpoint: str, **params) -> Optional[Dict[str, Any]]:
        ...
        try:
            return await asyncio.wait_for(
                self.clients.get_json(endpoint, **params), timeout=self.timeout
            )
    ...
    async def post_json(
        ...
        try:
            return await asyncio.wait_for(
                self.clients.post_json(endpoint, data), timeout=self.timeout
            )
        except asyncio.TimeoutError:
    ...
    async def put_json(
        ...
        try:
            return await asyncio.wait_for(
                self.clients.put_json(endpoint, data), timeout=self.timeout
            )
        except asyncio.TimeoutError:
