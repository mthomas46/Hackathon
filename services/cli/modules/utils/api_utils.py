...
class APIClient:
    ...
    async def get_json(self, endpoint: str, **params) -> Optional[Dict[str, Any]]:
        ...
        try:
            return await asyncio.wait_for(self.clients.get_json(endpoint, **params), timeout=self.timeout)
    ...
    async def post_json(self, endpoint: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            return await asyncio.wait_for(self.clients.post_json(endpoint, data), timeout=self.timeout)
        except asyncio.TimeoutError:
    ...
    async def put_json(self, endpoint: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            return await asyncio.wait_for(self.clients.put_json(endpoint, data), timeout=self.timeout)
        except asyncio.TimeoutError:
    ...
    async def delete(self, endpoint: str) -> bool:
        ...
        try:
            await asyncio.wait_for(self.clients.delete(endpoint), timeout=self.timeout)
            return True
        except asyncio.TimeoutError:
        ...
        for service_name, url in service_urls.items():
            try:
                ...
                if response and response.get("status") == "healthy":
                    results[service_name] = {"status": "healthy", "response": response}
                else:
                    results[service_name] = {"status": "unhealthy", "response": response}
            except Exception as e:
