"""Bedrock Proxy Service Adapter for CLI interaction Handles AWS Bedrock API
interactions and LLM model access."""
...
class BedrockProxyAdapter(BaseServiceAdapter):
    ...
    def _format_chat_response(self, data: Dict[str, Any]) -> None:
        ...
        if usage:
            self.console.print(
                f"[dim]📊 Token Usage: {usage.get('prompt_tokens', 0)} prompt + {usage.get('completion_tokens', 0)} completion = {usage.get('total_tokens', 0)} total[/dim]"
            )
