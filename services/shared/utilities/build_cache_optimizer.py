...
class BuildCacheOptimizer:
    ...
    def _parse_dockerfile_layers(self, lines: List[str]) -> List[DockerLayer]:
        ...
        for line in lines:
            ...
            if any(line.upper().startswith(cmd) for cmd in [
                'FROM', 'RUN', 'COPY', 'ADD', 'WORKDIR', 'ENV', 'ARG',
                'EXPOSE', 'VOLUME', 'USER', 'CMD', 'ENTRYPOINT'
            ]):
    ...
    def _analyze_layer_ordering(self, layers: List[DockerLayer]) -> List[CacheOptimization]:
        ...
        for idx, layer in enumerate(layers):
            if layer.instruction == 'RUN':
                run_content = ' '.join(layer.instruction + ' ' + '\n'.join([layer.instruction] if hasattr(layer, 'content') else [])).lower()
    ...
    def _analyze_dependency_separation(self, layers: List[DockerLayer]) -> List[CacheOptimization]:
        ...
        for idx, layer in enumerate(layers):
            if layer.instruction in ['COPY', 'ADD']:
                ...
                for dep in deps:
                    if any(dep_file in dep for dep_file in ['requirements.txt', 'package.json', 'go.mod', 'Cargo.toml']):
    ...
    def generate_optimized_dockerfile(self) -> Optional[str]:
        ...
        try:
            ...
            optimized_lines.append("# =========================================================================")
            optimized_lines.append(f"# Cache Efficiency Score: {self._calculate_cache_efficiency_score(self._parse_dockerfile_layers(lines)):.1f}/100")
            optimized_lines.append("#")
