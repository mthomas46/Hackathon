...
class SqlInjectionError(ValueError):
    """Exception raised when SQL identifier validation fails."""
    pass
...
def validate_sql_identifier(identifier: str) -> str:
    ...
    if not identifier or len(identifier) > 64:
        raise SqlInjectionError(f"Invalid SQL identifier: {identifier}")

    # Check for SQL reserved keywords
    if identifier.upper() in ['SELECT', 'INSERT', 'UPDATE', 'DELETE', 'DROP', 'CREATE', 'ALTER', 'TABLE', 'INDEX', 'VIEW']:
        raise SqlInjectionError(f"Reserved SQL keyword not allowed: {identifier}")

    # Must start with letter or underscore, followed by letters, digits, or underscores
    pattern = r"^[a-zA-Z_][a-zA-Z0-9_]*$"
    if not re.match(pattern, identifier):
        raise SqlInjectionError(f"Invalid SQL identifier format: {identifier}")

    return identifier


def sanitize_sql_identifier(identifier: str) -> str:
    """Validate and return SQL identifier, raising exception for invalid ones.

    This function performs the same validation as validate_sql_identifier
    but is named for backward compatibility.
