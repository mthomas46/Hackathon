...
class ServiceConfig(BaseSettings):
    ...
    service_name: str = Field(
        default="unknown-service", description="Unique service name"
    )
    ...
    database_pool_size: int = Field(
        default=10, gt=0, description="Database connection pool size"
    )
    ...
    # External services
    discovery_agent_url: Optional[str] = Field(
        None, description="Service discovery URL"
    )
    ...
    max_concurrent_requests: int = Field(
        default=100, gt=0, description="Max concurrent requests"
    )
    ...
    health_check_interval: int = Field(
        default=30, gt=0, description="Health check interval"
    )
...
class AnalysisServiceConfig(ServiceConfig):
    ...
    max_concurrent_analyses: int = Field(
        default=5, gt=0, description="Max concurrent analyses"
    )
    analysis_timeout: int = Field(
        default=300, gt=0, description="Analysis timeout in seconds"
    )
    ...
    # Storage settings
    analysis_results_ttl: int = Field(
        default=604800, gt=0, description="Analysis results TTL (seconds)"
    )
...
class OrchestratorConfig(ServiceConfig):
    ...
    max_concurrent_workflows: int = Field(
        default=50, gt=0, description="Max concurrent workflows"
    )
    workflow_timeout: int = Field(
        default=1800, gt=0, description="Workflow timeout in seconds"
    )
    ...
    enable_event_persistence: bool = Field(
        default=True, description="Enable event persistence"
    )
...
class PromptStoreConfig(ServiceConfig):
    ...
    max_prompt_size: int = Field(
        default=8192, gt=0, description="Max prompt size in characters"
    )
    enable_versioning: bool = Field(
        default=True, description="Enable prompt versioning"
    )

    # A/B testing settings
    ab_test_sample_size: int = Field(
        default=1000, gt=0, description="A/B test sample size"
    )
    ab_test_confidence_level: float = Field(
        default=0.95, gt=0, le=1, description="A/B test confidence level"
    )
...
class DiscoveryAgentConfig(ServiceConfig):
    ...
    discovery_interval: int = Field(
        default=30, gt=0, description="Service discovery interval"
    )
    heartbeat_timeout: int = Field(
        default=90, gt=0, description="Service heartbeat timeout"
    )

    # Registry settings
    max_services: int = Field(default=1000, gt=0, description="Max registered services")
    enable_service_auth: bool = Field(
        default=True, description="Enable service authentication"
    )

    # Monitoring settings
    metrics_retention: int = Field(
        default=604800, gt=0, description="Metrics retention period"
    )
...
class FrontendConfig(ServiceConfig):
    ...
    static_files_dir: str = Field(
        default="./static", description="Static files directory"
    )
...
class CLIConfig(ServiceConfig):
    ...
    max_command_timeout: int = Field(
        default=300, gt=0, description="Max command timeout"
    )
    enable_parallel_execution: bool = Field(
        default=True, description="Enable parallel command execution"
    )
    ...
    # History settings
    command_history_size: int = Field(
        default=1000, gt=0, description="Command history size"
    )
...
class SummarizerHubConfig(ServiceConfig):
    ...
    max_text_length: int = Field(
        default=50000, gt=0, description="Max text length for summarization"
    )
...
class DocStoreConfig(ServiceConfig):
    ...
    max_document_size: int = Field(
        default=10485760, gt=0, description="Max document size in bytes (10MB)"
    )
    ...
    enable_content_hashing: bool = Field(
        default=True, description="Enable content hashing for duplicates"
    )

    # Storage settings
    backup_interval: int = Field(
        default=3600, gt=0, description="Backup interval in seconds"
    )
    compression_enabled: bool = Field(
        default=True, description="Enable document compression"
    )
    max_versions_per_document: int = Field(
        default=10, gt=0, description="Max versions to keep per document"
    )

    # Search settings
    enable_full_text_search: bool = Field(
        default=True, description="Enable full-text search"
    )
    search_index_batch_size: int = Field(
        default=1000, gt=0, description="Search indexing batch size"
    )
    search_timeout: int = Field(default=30, gt=0, description="Search query timeout")

    # Analytics settings
    enable_usage_analytics: bool = Field(
        default=True, description="Enable usage analytics collection"
    )
    analytics_retention_days: int = Field(
        default=90, gt=0, description="Analytics data retention"
    )
    ...
    max_tags_per_document: int = Field(
        default=50, gt=0, description="Max tags per document"
    )
    ...
    max_bulk_operation_size: int = Field(
        default=1000, gt=0, description="Max items in bulk operations"
    )
    bulk_operation_timeout: int = Field(
        default=300, gt=0, description="Bulk operation timeout"
    )
...
def load_service_config(
    ...
    **overrides
    ...
    # Ensure service_name is set to service_type if not provided
    if "service_name" not in overrides:
        overrides["service_name"] = service_type

    return loader.load(
        lambda **kwargs: create_service_config(service_type, **kwargs), **overrides
    )
