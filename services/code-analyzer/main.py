...
async def startup_event():
    ...
    try:
        ...
        if logger_client:
            await logger_client.log_business_event("code_analyzer_startup", {
                "version": "0.1.0",
                "capabilities": ["code_analysis", "function_extraction", "class_analysis", "language_support", "prompt_generation"],
            ...
            await logger_client.log_info("Code Analyzer service started", {
...
async def analyze_code(request: CodeAnalysisRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        if logger_client:
            await logger_client.log_business_event("code_analysis_started", {
            ...
            await logger_client.log_info("Starting code analysis", {
                "request_id": request_id,
                "language": request.language,
                "code_lines": len(request.code.split('\n')),
                "analysis_scope": "functions" if request.include_functions else "classes" if request.include_classes else "minimal"
            })
        ...
        if logger_client:
            await logger_client.log_business_event("code_analysis_completed", {
                "request_id": request_id,
                "language": request.language,
        ...
        if logger_client:
            ...
            await logger_client.log_business_event("code_analysis_failed", {
                "request_id": request_id,
                "language": request.language if 'request' in locals() else None,
                "error_type": type(e).__name__,
                "error_message": str(e),
