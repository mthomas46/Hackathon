...
def render_resource_table(
    ...
    if show_capacity:
        ...
        if forecast_data:
            ...
            for resource_type, forecast in forecast_data.items():
                st.write(
                    f"- **{resource_type}**: {forecast['current']:.1f}% → {forecast['projected']:.1f}% (in {forecast_days} days)"
                )
    ...
    return {
        ...
        "allocation_summary": {
            ...
            "overall_utilization": (
                (filtered_df["allocated"].sum() / filtered_df["capacity"].sum()) * 100
                if filtered_df["capacity"].sum() > 0
                else 0
            ),
        },
