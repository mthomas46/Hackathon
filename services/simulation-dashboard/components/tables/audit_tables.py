...
def render_audit_event_details(
    event_row: pd.Series, on_investigate: Optional[Callable]
):
    ...
    with st.expander(
        f"{severity_icon} {event_type} - {action} ({timestamp})", expanded=False
    ):
...
def render_audit_compliance_report(audit_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    compliance_score = (
        100
        - (critical_events / total_events * 50)
        - (failed_events / total_events * 30)
    )
    ...
    if len(daily_stats) > 1:
        daily_stats["success_rate"] = (
            (daily_stats["total"] - daily_stats["failed"]) / daily_stats["total"]
        ) * 100

        # Simple trend analysis
        recent_success = daily_stats["success_rate"].tail(7).mean()
        previous_success = (
            daily_stats["success_rate"].head(-7).mean()
