...
def render_audit_table(
    ...
    for col in required_columns:
        if col not in df.columns:
            df[col] = (
                ...
                else "info" if col == "severity" else "success" if col == "status" else datetime.now()
            )
    ...
    if enable_filtering or enable_search:
        ...
        with filter_col2:
            event_type_filter = st.multiselect(
                "Event Type", options=df["event_type"].unique().tolist(), default=[], key="audit_event_filter"
            )

        with filter_col3:
            date_from = st.date_input("From Date", value=datetime.now() - timedelta(days=7), key="audit_date_from")

        with filter_col4:
            date_to = st.date_input("To Date", value=datetime.now(), key="audit_date_to")

        # Search
        if enable_search:
            search_term = st.text_input(
                "Search Events", placeholder="Search by user, action, or resource...", key="audit_search"
            )
    ...
    with col4:
        recent_count = len(
            filtered_df[pd.to_datetime(filtered_df["timestamp"]) > (datetime.now() - timedelta(hours=24))]
        )
    ...
    # Display columns
    display_columns = ["severity_icon", "timestamp_display", "event_type", "user", "action", "resource", "status_icon"]
    display_names = ["Severity", "Timestamp", "Event Type", "User", "Action", "Resource", "Status"]
    ...
    if not filtered_df.empty:
        ...
        high_priority_events = filtered_df[
            (filtered_df["severity"].isin(["critical", "high"])) | (filtered_df["status"] == "failed")
        ]
    ...
    if enable_export:
        ...
        with col_export1:
            export_format = st.selectbox("Format", options=["CSV", "JSON", "PDF"], key="audit_export_format")
    ...
    return {
        ...
        "event_breakdown": {"event_types": event_types.to_dict(), "severity_levels": severity_levels.to_dict()},
    }
...
def get_status_icon(status: str) -> str:
    ...
    icons = {"success": "✅", "failed": "❌", "warning": "⚠️", "pending": "⏳", "unknown": "❓"}
...
def render_audit_compliance_report(audit_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    daily_stats = (
        df.groupby("date")
        .agg({"id": "count", "status": lambda x: (x == "failed").sum(), "severity": lambda x: (x == "critical").sum()})
...
def generate_sample_audit_data(count: int = 50) -> List[Dict[str, Any]]:
    ...
    for i in range(count):
        timestamp = base_time + timedelta(
            days=np.random.randint(0, 7), hours=np.random.randint(0, 24), minutes=np.random.randint(0, 60)
        )
