...
def run_health_check(status_data: Dict[str, Any]):
    ...
    for service_name, service_info in services.items():
        ...
        is_healthy = (
            ...
            and cpu_usage < 90  # CPU usage < 90%
            and memory_usage < 90  # Memory usage < 90%
        )
    ...
    for service_name, result in health_results.items():
        if result["healthy"]:
            ...
            st.write(
                f"   Response: {result['response_time']:.0f}ms, CPU: {result['cpu_usage']:.1f}%, Memory: {result['memory_usage']:.1f}%"
            )
