"""
Context-Aware RAG System - PHASE 5: Enhanced with Pipeline
...
- Context filtering via pre-retrieval hooks
...
# ✨ PHASE 5: Import enhancement pipeline
...
class ContextAwareRAG:
    ...
    def __init__(self):
        ...
        # ✨ PHASE 5: Initialize enhancement pipeline
        try:
            self.enhancement_pipeline = EnhancementPipeline()
            self.use_enhancements = True
            logger.info("✅ ContextAwareRAG initialized with EnhancementPipeline (Phase 5)")
        except Exception as e:
            logger.warning(f"⚠️  EnhancementPipeline initialization failed: {e}. Using legacy mode.")
            self.enhancement_pipeline = None
            self.use_enhancements = False
            logger.info("ContextAwareRAG initialized (legacy mode)")
