...
class DocumentationOrchestrator:
    ...
    async def _generate_architecture_docs(
        ...
        # Placeholder - will be implemented by ArchitectureGenerator
        artifact = {
            'type': 'architecture',
...
Services: {analysis.total_services}

## Architecture Pattern
Pattern: {analysis.architecture.primary_pattern.name if analysis.architecture and analysis.architecture.primary_pattern else 'Unknown'}
Modularity Score: {analysis.modularity_score:.2f}

## Technology Stack
Languages: {', '.join(analysis.technology_stack.languages.keys()) if analysis.technology_stack else 'N/A'}
Frameworks: {', '.join(analysis.technology_stack.frameworks.keys()) if analysis.technology_stack else 'N/A'}

---
*Generated by Ecosystem MCP*
""",
            'format': 'markdown',
            'word_count': 150
        }
    ...
    async def _generate_component_docs(
        ...
        # Generate docs for each service/component
        if analysis.service_map:
            for service in analysis.service_map.services[:5]:  # Limit for now
                artifact = {
                    'type': 'component',
                    'title': f'Component: {service.name}',
                    'component_name': service.name,
                    'content': f"""# Component: {service.name}

## Overview
Path: {service.root_path}
Files: {service.file_count}

## Technology
Languages: {', '.join(service.languages)}
Frameworks: {', '.join(service.frameworks)}

## API
Has API: {'Yes' if service.has_api else 'No'}
Endpoints: {len(service.endpoints) if service.endpoints else 0}
