...
class PassMetrics:
    ...
    total_queries: int = 0
    cache_hits: int = 0
    total_answer_length: int = 0


@dataclass
class SectionMetrics:
    """Metrics for a complete section."""
    section_name: str
    start_time: float
    end_time: float
    duration_seconds: float
...
class MultiPassDocGenerator:
    """Generate deep, comprehensive documentation using multi-pass workflow with metrics."""
    ...
    def __init__(self, base_url: str = BASE_URL):
        ...
        self.current_section_metrics = None
        self.current_pass_metrics = None
    ...
    async def ask_rag(
        ...
        if (Path.cwd() / ".stop_generation").exists():
            print("\n\n🛑 STOP SIGNAL DETECTED - Cancelling generation...")
            raise KeyboardInterrupt("Stop signal file detected")
    ...
    async def multi_pass_section(
        ...
        # Truncate content for synthesis to fit API limits (max 500 chars question)
        # Instead of including all content in prompt, just use a summary approach
