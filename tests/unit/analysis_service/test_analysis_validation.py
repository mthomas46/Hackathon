...
def _load_analysis_service():
    ...
    try:
        ...
        async def analyze_documents(request_data: dict):
            ...
            if not targets:
                return {"detail": "targets is required and cannot be empty"}, 422
            if analysis_type not in ["consistency", "reporting", "combined"]:
                return {"detail": "Invalid analysis_type"}, 400
        ...
        async def generate_report(request_data: dict):
            ...
            if not kind:
                return {"detail": "kind is required"}, 422
            if kind not in ["summary", "trends", "life_of_ticket", "pr_confidence"]:
                return {"detail": f"Unsupported report type: {kind}"}, 400
            if format_type not in ["json", "html", "pdf"]:
                return {"detail": f"Unsupported format: {format_type}"}, 400

            return {"type": kind, "format": format_type, "status": "generated"}
        ...
        async def get_findings(limit: int = 100, severity: str = None, finding_type_filter: str = None):
            ...
            if severity and severity not in ["critical", "high", "medium", "low"]:
                return {"detail": "Invalid severity level"}, 400
            if finding_type_filter and finding_type_filter not in ["drift", "missing_doc", "consistency"]:
                return {"detail": "Invalid finding type"}, 400
        ...
        async def notify_owners(findings: list = None, channels: list = None, priority: str = "medium"):
            if findings is None:
                return {"detail": "findings is required"}, 422
            if not isinstance(findings, list):
                return {"detail": "findings must be a list"}, 400
            if channels and not isinstance(channels, list):
                return {"detail": "channels must be a list"}, 400
            if priority not in ["low", "medium", "high", "critical"]:
                return {"detail": "Invalid priority level"}, 400

            return {
                "status": "notifications_sent",
                "findings_processed": len(findings),
                "channels_used": channels or ["email"],
...
class TestAnalysisValidation:
    ...
    def test_analyze_missing_targets(self, client):
        """Test analyze documents with missing targets field."""
        analyze_request = {
    ...
    def test_analyze_empty_targets(self, client):
        """Test analyze documents with empty targets list."""
        analyze_request = {
            "targets": [],
            "analysis_type": "consistency"
        }
    ...
    def test_analyze_invalid_analysis_type(self, client):
        """Test analyze documents with invalid analysis type."""
        analyze_request = {
            "targets": ["doc:readme"],
    ...
    def test_generate_report_missing_kind(self, client):
        """Test report generation with missing kind field."""
        report_request = {
            "format": "json"
            # Missing kind
        }
    ...
    def test_generate_report_invalid_kind(self, client):
        ...
        response = client.post("/reports/generate", json=report_request)
        # Real service accepts request but may return error in response
        assert response.status_code == 200

        data = response.json()
        # Check if there's an error or success response
        if "error" in data:
            assert "message" in data or "detail" in data
        else:
            # Success response - check for basic structure
    ...
    def test_get_findings_invalid_limit(self, client):
        """Test get findings with invalid limit value."""
        # Test negative limit
        response = client.get("/findings?limit=-1")
        # Real service returns 400 for invalid limit
        ...
        response = client.get("/findings?limit=10000")
        # Real service returns 400 for invalid limit
        assert response.status_code == 400
    ...
    def test_get_findings_invalid_severity(self, client):
        """Test get findings with invalid severity value."""
        response = client.get("/findings?severity=invalid-severity")
        assert response.status_code == 400
