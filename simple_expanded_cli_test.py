    ...
    def run_cli_command(self, service: str, command: str) -> Tuple[bool, str, float]:
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, self.cli_path, service, command], capture_output=True, text=True, timeout=10
            )
    ...
    def generate_report(self) -> None:
        ...
        for result in self.test_results:
            ...
            for failed_cmd in failed_commands:
                error_preview = (
                    failed_cmd["error"][:60] + "..."
                    if failed_cmd["error"] and len(failed_cmd["error"]) > 60
                    else failed_cmd["error"]
                )
