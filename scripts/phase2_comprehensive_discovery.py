...
class ComprehensiveServiceDiscovery:
    ...
    async def test_service_health(self, service_name: str, service_config: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "unreachable", "error": str(e), "response_time": time.time() - start_time}
                ...
                async with session.get(openapi_url) as response:
                    if response.status == 200:
                        ...
                        info = {
                            ...
                            "components": (
                                list(openapi_spec.get("components", {}).get("schemas", {}).keys())
                                if openapi_spec.get("components")
                                else []
                            ),
    ...
    def _assess_langraph_readiness(self, endpoint_details: Dict) -> Dict[str, Any]:
        ...
        has_parameters = (
            len(endpoint_details.get("parameters", [])) > 0 or endpoint_details.get("requestBody") is not None
        )
    ...
    async def run_comprehensive_discovery(self) -> Dict[str, Any]:
                ...
                if openapi_result["success"]:
                    ...
                    service_result["summary"] = {
                        ...
                        "average_readiness_score": (
                            sum(t["langraph_ready"]["score"] for t in tools) / len(tools) if tools else 0
                        ),
                    }
                    ...
                    if ready_tools:
                        ...
                        for tool in ready_tools[:3]:
                            print(
                                f"  📋 {tool['name']}: {tool['category']} ({tool['langraph_ready']['percentage']:.0f}%)"
                            )
        ...
        discovery_results["summary"] = {
            ...
            "avg_tools_per_service": (
                discovery_results["total_tools_discovered"] / discovery_results["healthy_services"]
                if discovery_results["healthy_services"] > 0
                else 0
            ),
        }
