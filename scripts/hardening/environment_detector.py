...
class EnvironmentDetector:
    ...
    def detect_environment(self) -> EnvironmentProfile:
        ...
        if self._detection_cache and current_time - self._last_detection < self._cache_ttl:
    ...
    def _is_running_in_kubernetes(self) -> bool:
        ...
        return "KUBERNETES_SERVICE_HOST" in os.environ or os.path.exists("/var/run/secrets/kubernetes.io")

    def _gather_network_info(self) -> Dict[str, Any]:
        """Gather network connectivity information."""
        info = {"hostname": socket.gethostname(), "interfaces": {}, "connectivity": {}}
    ...
    def _create_http_config(self, environment: Environment, run_profile: RunProfile) -> HTTPConfig:
        ...
        if environment == Environment.DOCKER:
            ...
            base_config.update({"timeout": 10.0, "retries": 1})  # Faster for local development
    ...
    def _create_discovery_config(self, environment: Environment, run_profile: RunProfile) -> ServiceDiscoveryConfig:
        ...
        if environment == Environment.DOCKER:
            ...
            base_config.update({"use_dns": True, "health_check_interval": 10, "service_timeout": 2.0})

        # Adjust for run profile
        if run_profile == RunProfile.DEVELOPMENT:
            base_config.update({"health_check_interval": 60, "retry_attempts": 5})  # Less frequent in development
        elif run_profile == RunProfile.TESTING:
            base_config.update(
                {"use_health_checks": False, "service_timeout": 1.0, "retry_attempts": 1}  # Skip health checks in tests
            )
