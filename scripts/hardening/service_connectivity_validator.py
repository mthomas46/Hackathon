...
class ServiceConnectivityValidator:
    ...
    def _load_service_dependencies(self) -> Dict[str, ServiceDependency]:
        ...
        return {
            "doc_store": ServiceDependency(
                service="doc_store", depends_on=["redis"], critical=True  # Uses Redis for caching
            ),
            "orchestrator": ServiceDependency(
                service="orchestrator", depends_on=["redis"], critical=True  # Uses Redis for coordination
            ),
            ...
            "discovery-agent": ServiceDependency(
                service="discovery-agent", depends_on=["orchestrator"], critical=True  # Registers with orchestrator
            ),
            ...
            "summarizer-hub": ServiceDependency(
                service="summarizer-hub", depends_on=["llm-gateway"], critical=True  # Uses LLM for summarization
            ),
            "notification-service": ServiceDependency(
                service="notification-service", depends_on=[], critical=False  # Standalone service
            ),
            "prompt_store": ServiceDependency(
                service="prompt_store", depends_on=["doc_store"], critical=False  # May store results in doc_store
            ),
        }
    ...
    def _load_integration_tests(self) -> List[IntegrationTest]:
        ...
        return [
            ...
            IntegrationTest(
                ...
                test_data={"document_id": "test_doc_id", "analysis_types": ["basic_analysis"]},
                expected_status=200,
            ),
            IntegrationTest(
    ...
    async def validate_docker_port_mappings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode != 0:
                return {"port_validation": {}, "error": "Could not query Docker containers"}
            ...
            return {"port_validation": {}, "error": str(e)}
    ...
    async def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        total_checks = (
            dependency_results["total_dependencies"] + integration_results["total_tests"] + 1  # Port validation
        )
    ...
    def _identify_critical_connectivity_issues(
        self, dependency_results: Dict, integration_results: Dict, port_validation: Dict
    ) -> List[str]:
