...
class ProductionReadinessValidator:
                    ...
                    if line.strip() and ("hackathon-" in line or "hackathon_" in line):
                        ...
                        if len(parts) >= 2:
                            ...
                            if docker_healthy:
                                ...
                                service_name = (
                                    container_name.replace("hackathon-", "").replace("hackathon_", "").replace("-1", "")
                                )
    ...
    def validate_service_connectivity(self) -> Dict[str, Any]:
        ...
        for service, port in services.items():
            try:
                ...
                connectivity_results[service] = {"reachable": False, "error": str(e), "response_time_ms": 0}
    ...
    def validate_port_mappings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"passed": False, "error": str(e), "port_conflicts": [], "port_mappings": {}}
    ...
    def validate_api_schemas(self) -> Dict[str, Any]:
        ...
        return {"passed": len(schema_issues) == 0, "schema_issues": schema_issues, "issues_found": len(schema_issues)}
    ...
    def validate_error_handling(self) -> Dict[str, Any]:
        ...
        for test in test_cases:
            try:
                ...
                if e.code >= 500:
                    error_handling_issues.append(
                        {"service": test["service"], "issue": f"Server error instead of client error: {e.code}"}
                    )
                # 4xx errors are perfectly acceptable for invalid endpoints
            except Exception as e:
                # Connection issues are more concerning than API validation
                error_handling_issues.append(
                    {"service": test["service"], "issue": f"Service unreachable for error handling test: {str(e)}"}
                )
    ...
    def validate_workflows(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if e.code >= 500:
                workflow_issues.append(
                    {"workflow": "document_creation", "issue": f"Document creation failed with server error: {e.code}"}
                )
            # 400-level errors are API validation issues, not workflow failures
        except Exception as e:
            workflow_issues.append(
                {"workflow": "document_creation", "issue": f"Document creation workflow failed: {str(e)}"}
            )
        ...
        try:
            ...
            workflow_issues.append({"workflow": "health_check", "issue": f"Health check workflow failed: {str(e)}"})
    ...
    def validate_dependencies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            dependency_issues.append(
                {"dependency": "redis", "issue": "Redis not reachable, affects doc_store and orchestrator"}
            )
        ...
        try:
            with urllib.request.urlopen("http://localhost:11434/api/tags", timeout=5) as response:
                if response.getcode() != 200:
                    dependency_issues.append({"dependency": "ollama", "issue": "Ollama API not responding properly"})
        except Exception:
            dependency_issues.append(
                {"dependency": "ollama", "issue": "Ollama not reachable, affects LLM Gateway functionality"}
            )
    ...
    def validate_health_checks(self) -> Dict[str, Any]:
        ...
        for service in test_services:
            port_map = {"doc_store": 5087, "llm-gateway": 5055, "analysis-service": 5080, "discovery-agent": 5045}
        ...
        critical_discrepancies = [
            d for d in health_discrepancies if "impact" not in d or "critical" in d.get("impact", "")
        ]
    ...
    def _generate_readiness_recommendations(
        ...
        production_required_failures: List[str],
    ) -> List[str]:
        ...
        if high_failures:
            recommendations.append(
                f"MEDIUM: Address {len(high_failures)} high-priority issues for production deployment"
            )

        if medium_failures:
            recommendations.append(
                f"LOW: Consider fixing {len(medium_failures)} medium-priority issues for optimal production readiness"
            )
    ...
    def print_readiness_report(self, results: Dict[str, Any]):
        ...
        for check_name, result in results["validation_results"].items():
            ...
            categories[category]["checks"].append(
                {"name": check_name, "passed": result["passed"], "severity": result["check"].severity}
            )
