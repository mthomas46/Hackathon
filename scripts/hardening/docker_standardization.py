...
class DockerStandardizationManager:
    ...
    def _load_services_config(self) -> Dict[str, ServiceConfig]:
        ...
        return {
            "redis": ServiceConfig(
                ...
                dependencies=[],
                environment_vars={},
                health_check={"test": ["CMD", "redis-cli", "ping"], "interval": "30s", "timeout": "10s", "retries": 3},
                profiles=["core", "development", "production"],
            ),
            "doc_store": ServiceConfig(
                name="doc_store",
                port_config=ServicePortConfig(
                    service_name="doc_store", external_port=5087, internal_port=5010, health_endpoint="/health"
                ),
                dependencies=["redis"],
                environment_vars={"SERVICE_PORT": "5010", "REDIS_URL": "redis://redis:6379"},
                health_check={
            ...
            "orchestrator": ServiceConfig(
                name="orchestrator",
                port_config=ServicePortConfig(
                    service_name="orchestrator", external_port=5099, internal_port=5099, health_endpoint="/health"
                ),
                dependencies=["redis"],
                environment_vars={"SERVICE_PORT": "5099", "REDIS_URL": "redis://redis:6379"},
                health_check={
            ...
            "llm-gateway": ServiceConfig(
                name="llm-gateway",
                port_config=ServicePortConfig(
                    service_name="llm-gateway", external_port=5055, internal_port=5055, health_endpoint="/health"
                ),
                dependencies=["ollama"],
                environment_vars={"SERVICE_PORT": "5055", "OLLAMA_ENDPOINT": "http://ollama:11434"},
                health_check={
            ...
            "discovery-agent": ServiceConfig(
                ...
                port_config=ServicePortConfig(
                    service_name="discovery-agent", external_port=5045, internal_port=5045, health_endpoint="/health"
                ),
                dependencies=["orchestrator"],
                environment_vars={"SERVICE_PORT": "5045", "ORCHESTRATOR_URL": "http://orchestrator:5099"},
                health_check={
            ...
            "notification-service": ServiceConfig(
                ...
                dependencies=[],
                environment_vars={"SERVICE_PORT": "5020"},
                health_check={
                    "test": ["CMD", "curl", "-f", "http://localhost:5020/health"],
                    "interval": "30s",
                    "timeout": "10s",
            ...
            "code-analyzer": ServiceConfig(
                name="code-analyzer",
                port_config=ServicePortConfig(
                    service_name="code-analyzer", external_port=5025, internal_port=5025, health_endpoint="/health"
                ),
                dependencies=[],
                environment_vars={"SERVICE_PORT": "5025"},
                health_check={
            ...
            "source-agent": ServiceConfig(
                ...
                environment_vars={"SERVICE_PORT": "5070", "DOC_STORE_URL": "http://doc_store:5010"},
                health_check={
            ...
            "frontend": ServiceConfig(
                name="frontend",
                port_config=ServicePortConfig(
                    service_name="frontend", external_port=3000, internal_port=3000, health_endpoint="/"
                ),
                dependencies=["orchestrator"],
                environment_vars={"REACT_APP_API_URL": "http://localhost:5099"},
                health_check={
            ...
            "ollama": ServiceConfig(
                name="ollama",
                port_config=ServicePortConfig(
                    service_name="ollama", external_port=11434, internal_port=11434, health_endpoint="/api/tags"
                ),
                dependencies=[],
                environment_vars={},
                health_check={
    ...
    def validate_port_consistency(self) -> Dict[str, Any]:
        ...
        validation_results = {"port_conflicts": [], "configuration_issues": [], "recommendations": []}
        ...
        for service_name, config in self.services_config.items():
            ...
            if ext_port in used_ports:
                validation_results["port_conflicts"].append(
                    {"port": ext_port, "services": [used_ports[ext_port], service_name], "severity": "critical"}
                )
            else:
    ...
    def generate_docker_compose(self, environment: str = "development") -> str:
        ...
        compose_config = {
            "version": "3.8",
            "services": {},
            "networks": {"hackathon": {"driver": "bridge"}},
            "profiles": {},
        }
    ...
    def _generate_service_definition(self, config: ServiceConfig, environment: str) -> Dict[str, Any]:
        ...
        if environment == "production":
            ...
            service_def["logging"] = {"driver": "json-file", "options": {"max-size": "10m", "max-file": "3"}}

        return service_def

    def update_service_dockerfiles(self) -> Dict[str, Any]:
        """Update service Dockerfiles to match port configurations"""
        update_results = {"updated_services": [], "errors": [], "recommendations": []}
        ...
        for service_name, config in self.services_config.items():
            ...
            if os.path.exists(dockerfile_path):
                try:
                    ...
                    updated_content = self._update_dockerfile_ports(dockerfile_content, config.port_config)
                    ...
                    update_results["errors"].append({"service": service_name, "error": str(e)})
            else:
                update_results["errors"].append({"service": service_name, "error": "Dockerfile not found"})
        ...
        for line in lines:
            ...
            # Update health check URL
                ...
                if port_config.health_endpoint != "/":
                    ...
                    updated_line = line.replace("http://localhost:[0-9]+/[^ ]*", health_url)
    ...
    def export_configurations(
        self, output_dir: str = "/Users/mykalthomas/Documents/work/Hackathon/config/standardized"
    ):
