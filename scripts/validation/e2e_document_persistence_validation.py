...
class DocumentPersistenceValidator:
    ...
    async def _validate_service_integration(self) -> List[ValidationResult]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            try:
                ...
                test_query = {"query": "test integration query", "format": "json"}

                async with session.post(
                    f"{self.services['interpreter']}/execute-query",
                    json=test_query,
            ...
            start_time = time.time()
            try:
                test_llm_query = {"prompt": "Hello, this is a test query", "max_tokens": 50}

                async with session.post(
                    f"{self.services['llm_gateway']}/query",
                    json=test_llm_query,
        ...
        async with aiohttp.ClientSession() as session:
            for i, test_case in enumerate(self.test_queries[:2]):  # Test first 2 queries
                ...
                try:
                    ...
                    interpret_payload = {"query": test_case["query"], "user_id": f"test_user_{i}"}

                    async with session.post(
                        f"{self.services['interpreter']}/interpret-query",
                        ...
                        if response.status == 200:
                            ...
                            results.append(
                                ValidationResult(
                                    ...
                                    warnings=(
                                        []
                                        if (has_intent and has_entities and has_confidence)
                                        else ["Missing required response fields"]
                                    ),
                                )
                            )
                        else:
    ...
    async def _validate_workflow_execution(self) -> List[ValidationResult]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            try:
                async with session.get(
                    f"{self.services['interpreter']}/workflows/templates", timeout=aiohttp.ClientTimeout(total=10)
                ) as response:
                    duration = time.time() - start_time
            ...
            try:
                ...
                async with session.post(
                    f"{self.services['interpreter']}/workflows/execute-direct",
    ...
    async def _validate_document_generation(self) -> List[ValidationResult]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            try:
                async with session.get(
                    f"{self.services['interpreter']}/outputs/formats", timeout=aiohttp.ClientTimeout(total=5)
                ) as response:
                    duration = time.time() - start_time
        ...
        async with aiohttp.ClientSession() as session:
            for doc_info in self.generated_documents:
                ...
                try:
                    ...
                    async with session.get(
                        f"{self.services['doc_store']}/documents/{document_id}", timeout=aiohttp.ClientTimeout(total=10)
                    ) as response:
                        duration = time.time() - start_time
        ...
        async with aiohttp.ClientSession() as session:
            for doc_info in self.generated_documents:
                ...
                try:
                    ...
                    async with session.get(
                        f"{self.services['interpreter']}/documents/{document_id}/provenance",
                            ...
                            results.append(
                                ValidationResult(
                                    ...
                                    details={
                                        ...
                                        "execution_id": provenance_data.get("workflow_execution", {}).get(
                                            "execution_id"
                                        ),
                                    },
                                )
        ...
        async with aiohttp.ClientSession() as session:
            for doc_info in self.generated_documents:
                ...
                try:
                    ...
                    async with session.get(
                        f"{self.services['doc_store']}/documents/{document_id}/download",
                        timeout=aiohttp.ClientTimeout(total=15),
                    ) as response:
                        duration = time.time() - start_time
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    ...
                    if response.status in [200, 202]:
                        ...
                        if execution_id:
                            async with session.get(
                                f"{self.services['interpreter']}/workflows/{execution_id}/trace",
                                timeout=aiohttp.ClientTimeout(total=10),
                            ) as trace_response:
                        ...
                        if document_id:
                            async with session.get(
                                f"{self.services['doc_store']}/documents/{document_id}",
                                timeout=aiohttp.ClientTimeout(total=10),
                            ) as doc_response:
    ...
    async def _validate_performance_reliability(self) -> List[ValidationResult]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for i in range(5):
                ...
                try:
                    async with session.get(
                        f"{self.services['interpreter']}/health", timeout=aiohttp.ClientTimeout(total=5)
                    ) as response:
                        if response.status == 200:
            ...
            try:
                ...
                for i in range(3):
                    tasks.append(
                        session.get(f"{self.services['interpreter']}/intents", timeout=aiohttp.ClientTimeout(total=10))
                    )
    ...
    def display_results(self, suite: E2EValidationSuite, format_type: str = "table"):
        ...
        for result in suite.results:
            ...
            table.add_row(result.test_name, status_text, f"{result.duration:.3f}s", details_str)
        ...
        if self.generated_documents:
            ...
            for doc in self.generated_documents:
                docs_table.add_row(
                    ...
                    (
                        doc.get("document_id", "N/A")[:20] + "..."
                        if doc.get("document_id") and len(doc.get("document_id", "")) > 20
                        else doc.get("document_id", "N/A")
                    ),
                    (
                        doc.get("execution_id", "N/A")[:20] + "..."
                        if doc.get("execution_id") and len(doc.get("execution_id", "")) > 20
                        else doc.get("execution_id", "N/A")
                    ),
                )
