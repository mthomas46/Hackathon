...
class OrchestratorRegistrationProof:
    ...
    def log_result(self, test_name: str, success: bool, details: str = ""):
        ...
        self.test_results.append({"test": test_name, "success": success, "details": details, "timestamp": timestamp})
    ...
    def test_enhanced_url_normalization(self) -> bool:
        ...
        for case in test_cases:
            ...
            self.log_result(case["name"], test_passed, f"{input_url} → {normalized}")

        return self.log_result(
            "URL Normalization Capability", all_passed, "Enhanced Discovery Agent can handle Docker network URLs"
        )
    ...
    def test_service_discovery_with_networking(self) -> bool:
        ...
        for service in test_services:
            try:
                ...
                response = requests.post(f"{self.discovery_agent_url}/discover", json=payload, timeout=15)
                ...
                if response.status_code == 200:
                    ...
                    if discovery_data.get("success", False):
                        ...
                        self.log_result(
                            f"Discover {service['name']}", True, f"Found {endpoints_count} endpoints via Docker network"
                        )
                    else:
                        self.log_result(
                            f"Discover {service['name']}", False, discovery_data.get("message", "Discovery failed")
                        )
                else:
                    self.log_result(
                        f"Discover {service['name']}", False, f"HTTP {response.status_code}: {response.text[:100]}"
                    )
    ...
    def test_orchestrator_registration_capability(self) -> bool:
        ...
        try:
            ...
            registry_health_response = requests.get(
                f"{self.orchestrator_url}/api/v1/service-registry/health", timeout=10
            )
            ...
            registration_response = requests.post(
                f"{self.orchestrator_url}/api/v1/service-registry/register", json=test_registration_payload, timeout=10
            )
            ...
            if registration_success:
                ...
                retrieval_response = requests.get(
                    f"{self.orchestrator_url}/api/v1/service-registry/services/test_discovery_proof", timeout=10
                )
    ...
    def demonstrate_complete_workflow(self) -> bool:
        ...
        for i, step in enumerate(workflow_steps, 1):
            try:
                ...
                self.log_result(f"Workflow Step {i}: {step['step']}", step_valid, step["description"])
