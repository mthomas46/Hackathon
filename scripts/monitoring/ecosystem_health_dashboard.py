...
class HealthMonitor:
    ...
    async def check_all_services(self) -> EcosystemHealth:
        ...
        async with aiohttp.ClientSession() as session:
            tasks = [self.check_service_health(session, name, config) for name, config in self.services.items()]
    ...
    def _generate_alerts(self, service_healths: List[ServiceHealth]) -> List[str]:
        ...
        healthy_percentage = (len([s for s in service_healths if s.status == "healthy"]) / len(service_healths)) * 100
    ...
    def display_health_table(self, ecosystem_health: EcosystemHealth):
        ...
        if ecosystem_health.alerts:
            alerts_panel = Panel.fit("\n".join(ecosystem_health.alerts), title="🚨 Alerts", border_style="red")
            console.print(alerts_panel)

    def get_historical_data(self, service_name: Optional[str] = None, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        with Live(layout, refresh_per_second=1, screen=True):
            while True:
                try:
                    ...
                    health_percentage = (ecosystem_health.healthy_services / ecosystem_health.total_services) * 100
                    header_color = (
                        "green" if health_percentage >= 95 else "yellow" if health_percentage >= 90 else "red"
                    )
                    ...
                    for service in sorted(ecosystem_health.services, key=lambda s: s.name):
                        status_emoji = {"healthy": "✅", "unhealthy": "❌", "timeout": "⏰", "error": "💥"}.get(
                            service.status, "❓"
                        )
                    ...
                    footer_text = (
                        f"Last updated: {ecosystem_health.timestamp.strftime('%H:%M:%S')} | "
    ...
    def _generate_recommendations(self, ecosystem_health: EcosystemHealth, stats: Dict[str, Any]) -> List[str]:
        ...
        for service in ecosystem_health.services:
            if service.status != "healthy":
                recommendations.append(f"🔧 Investigate {service.name}: {service.error_message}")
