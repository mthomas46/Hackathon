...
class AlertManager:
    ...
    def evaluate_alerts(
        self, service_status: ServiceHealthStatus, previous_status: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_alert_message(self, rule: AlertRule, service_status: ServiceHealthStatus) -> str:
        ...
        if rule.name == "service_down":
            ...
            return (
                f"Service {service_status.service_name} has {service_status.consecutive_failures} consecutive failures"
            )
...
Please investigate and resolve the issue.
    ...
    async def _send_slack_notification(self, alert: Dict[str, Any]):
        ...
        try:
            ...
            color = {"critical": "#FF0000", "warning": "#FFA500", "info": "#00FF00"}.get(alert["severity"], "#808080")
...
class AutomatedHealthMonitor:
    ...
    async def process_health_results(self, health_statuses: List[ServiceHealthStatus]):
        ...
        for status in health_statuses:
            ...
            self.database.record_health_check(
                status.service_name, status.status, status.response_time, status.status_code, status.error_message
            )

            # Update service status
            self.database.update_service_status(status.service_name, status.status, consecutive_failures)
            ...
            for alert in alerts:
                self.database.record_alert(
                    alert["service_name"], alert["rule_name"], alert["severity"], alert["message"]
                )
    ...
    async def generate_health_report(self) -> Dict[str, Any]:
        ...
        overall_health = (
            ...
            else "degraded" if unhealthy_services < total_services * 0.3 else "critical"
        )
