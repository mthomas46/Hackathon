...
class ComprehensiveDockerTester:
    ...
    def __init__(self, project_root: str = None):
        ...
        self.services = {
            # Infrastructure
            "redis": ServiceConfig(
                name="Redis", port=6379, health_endpoint="ping", dependencies=[], profile="core", build_type="image"
            ),
            # Core Services
            ...
            "frontend": ServiceConfig(
                ...
                port=3000,
                health_endpoint="/health",
                dependencies=["orchestrator", "analysis-service"],
                profile="core",
                build_type="image",
            ),
            # AI Services
    ...
    def run_command(self, cmd: str, timeout: int = 300) -> Tuple[bool, str, str]:
        ...
        try:
            result = subprocess.run(
                cmd, shell=True, cwd=self.project_root, capture_output=True, text=True, timeout=timeout
            )
    ...
    def is_container_running(self, service_id: str) -> bool:
        ...
        success, stdout, stderr = self.run_command(
            f"docker ps --filter name=hackathon-{service_id.replace('_', '-')}-1 --format '{{{{.Status}}}}'"
        )
    ...
    async def test_individual_service(self, service_id: str) -> Dict:
        ...
        with Progress(SpinnerColumn(), TextColumn(f"Testing {service.name}..."), console=console) as progress:
            task = progress.add_task("", total=None)

            try:
                # Start service and dependencies
                deps = " ".join(service.dependencies + [service_id])
                success, stdout, stderr = self.run_command(f"docker compose -f {self.compose_file} up -d {deps}")
    ...
    async def test_profile_services(self, profile: str) -> Dict:
        ...
        profile_services = [service_id for service_id, service in self.services.items() if service.profile == profile]
    ...
    async def test_full_ecosystem(self) -> Dict:
        ...
        try:
            ...
            success, stdout, stderr = self.run_command(f"docker compose -f {self.compose_file} {profile_args} up -d")
    ...
    def generate_report(self):
        ...
        for service_id, result in self.test_results.items():
            ...
            table.add_row(self.services.get(service_id, {}).get("name", service_id), status, health, issues, duration)
    ...
    async def run_individual_tests(self):
        ...
        test_order = [
            ...
            "bedrock-proxy",
            "github-mcp",  # AI (no custom Dockerfile)
            "memory-agent",
            "discovery-agent",  # Development
            "notification-service",
            "code-analyzer",
            "secure-analyzer",
            "log-collector",  # Production
        ]
    ...
    async def run_ecosystem_test(self):
        ...
        status = "✅" if result["success"] else "❌"
        console.print(
            f"{status} Ecosystem: {len(result['healthy_services'])}/{result['total_services']} services healthy"
        )
