...
class InterServiceCommunicationTester:
    ...
    async def test_individual_service_endpoints(self):
        ...
        test_cases = [
            ...
            (
                ...
                "POST",
                "/detect",
                {"content": "password = 'admin123'", "language": "python"},
                "Security detection",
            ),
            # Log Collector
            ...
            (
                ...
                "POST",
                "/logs",
                {"level": "INFO", "message": "Test log message", "service": "test_service"},
                "Log submission",
            ),
            ...
            (
                ...
                "POST",
                "/summarize",
                {"content": "This is a test document for summarization.", "format": "text"},
    ...
    async def test_cross_service_communication(self):
        ...
        scenarios = [
            {
                ...
                "steps": [
                    (
                        ...
                        "POST",
                        "/analyze",
                        {"code": "import os\npassword = os.getenv('SECRET')", "language": "python"},
                    ),
                    (
                        "secure_analyzer",
                        "POST",
        ...
        for scenario in scenarios:
            ...
            scenario_result = {"name": scenario["name"], "steps": [], "success": True, "total_time": 0}
            ...
            for i, (service, method, endpoint, data) in enumerate(scenario["steps"]):
                ...
                step_result = await self._test_endpoint(
                    service, method, endpoint, data, f"Step {i+1}: {service} {endpoint}"
                )
    ...
    async def test_service_workflows(self):
        ...
        for workflow in workflows:
            ...
            workflow_result = {"name": workflow["name"], "steps": [], "success": True, "total_time": 0}
            ...
            for i, (service, method, endpoint, data) in enumerate(workflow["steps"]):
                ...
                step_result = await self._test_endpoint(service, method, endpoint, data, f"Workflow step {i+1}")
    ...
    async def test_performance_and_load(self):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for service, method, endpoint in concurrent_tests:
                future = executor.submit(self._sync_test_endpoint, service, method, endpoint, None)
    ...
    async def _test_endpoint(
        self, service: str, method: str, endpoint: str, data: Optional[Dict] = None, description: str = ""
    ) -> Dict[str, Any]:
    ...
    def _sync_test_endpoint(
        self, service: str, method: str, endpoint: str, data: Optional[Dict] = None
    ) -> Dict[str, Any]:
    ...
    def _display_individual_test_results(self):
        ...
        for result in self.results["individual_tests"]:
            ...
            table.add_row(
                result["service"], result["endpoint"], result["method"], status, time_str, result.get("description", "")
            )
    ...
    def _display_cross_service_results(self):
        ...
        for result in self.results["cross_service_tests"]:
            ...
            table.add_row(result["name"], str(total_steps), f"{status_icon} {success_rate}", time_str)
