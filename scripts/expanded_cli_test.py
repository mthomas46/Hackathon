        ...
        if "/health" in url:
            ...
            return {
                "memories": [
                    ...
                    {
                        "id": "mem2",
                        "content": "Discussed service architecture",
                        "context_id": "conv1",
            ...
            return {
                "repositories": [{"name": "test-repo", "full_name": "user/test-repo", "description": "Test repository"}]
            }
        else:
...
class ExpandedCLITester:
    ...
    async def test_service_adapter(
        self, service_name: str, base_url: str, adapter_class: Any, test_commands: List[str]
    ) -> None:
    ...
    async def test_frontend(self) -> None:
        ...
        await self.test_service_adapter(
            "frontend", "http://hackathon-frontend-1:3000", FrontendAdapter, ["status", "pages", "test-ui", "stats"]
        )
    ...
    async def test_cross_service_integration(self) -> None:
        ...
        for test_name in integration_tests:
            ...
            try:
                ...
                test_result = TestResult(
                    service_name="integration", command=test_name, success=success, execution_time=execution_time
                )
                self.test_results.append(test_result)
