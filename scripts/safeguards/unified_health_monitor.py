...
class UnifiedHealthMonitor:
    ...
    def _execute_health_check(self, config: HealthCheckConfig, method: HealthCheckMethod) -> HealthResult:
    ...
    def _check_redis_ping(self, config: HealthCheckConfig, start_time: float) -> HealthResult:
        ...
        try:
            r = redis.Redis(
                host="localhost", port=config.host_port, decode_responses=True, socket_timeout=config.timeout
            )
            ping_result = r.ping()
            response_time = (time.time() - start_time) * 1000
    ...
    def print_health_report(self, results: Dict[str, HealthResult]):
        ...
        print(
            f"Healthy: {report['healthy_services']} | Unhealthy: {report['unhealthy_services']} | Degraded: {report['degraded_services']} | Unreachable: {report['unreachable_services']}"
        )
        ...
        for service_name, result in results.items():
            ...
            print(
                f"  {status_emoji} {service_name:20} | {result.status.value:12} | {result.method_used.value:15} | {result.response_time_ms:6.1f}ms"
            )
