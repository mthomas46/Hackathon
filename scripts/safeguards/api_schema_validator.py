...
class APISchemaValidator:
    ...
    def _load_service_schemas(self) -> Dict[str, Dict[str, Type[BaseModel]]]:
        ...
        return {
            ...
            "orchestrator": {"/api/v1/services": ListResponseModel, "/health": HealthResponseModel},
            "llm-gateway": {"/api/v1/providers": ListResponseModel, "/health": HealthResponseModel},
            "discovery-agent": {"/api/v1/discovery/services": ListResponseModel, "/health": HealthResponseModel},
            "analysis-service": {
                "/api/v1/analysis/analyze": SuccessResponseModel,
                "/api/v1/analysis/capabilities": ListResponseModel,
                "/health": HealthResponseModel,
            },
            "prompt_store": {"/api/v1/prompts": ListResponseModel, "/health": HealthResponseModel},
        }

    def validate_response(self, service_name: str, endpoint: str, response_data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(is_valid=True, errors=[], warnings=[], service_name=service_name, endpoint=endpoint)
            ...
            def wrapper(*args, **kwargs):
                try:
                    ...
                    if isinstance(result, dict):
                        validation_result = self.validator.validate_response(service_name, endpoint, result)

                        if not validation_result.is_valid:
                            logger.error(
                                f"Schema validation failed for {service_name}{endpoint}: {validation_result.errors}"
                            )
...
def test_doc_store_response():
    ...
    invalid_response = {
        ...
        "data": {"items": [{"id": "test", "content": "test"}], "total": 1, "has_more": False},
