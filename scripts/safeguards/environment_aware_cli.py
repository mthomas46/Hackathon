...
class EnvironmentDetector:
    ...
    def get_environment_info(self) -> Dict[str, any]:
        ...
        return {
            ...
            "service_endpoints": {name: self.get_service_url(name) for name in self.service_endpoints.keys()},
...
class EnvironmentAwareCLI:
    ...
    def execute_service_command(
        self, service_name: str, endpoint: str, method: str = "GET", data: Optional[Dict] = None
    ) -> Tuple[bool, str]:
    ...
    def health_check_all_services(self) -> Dict[str, Dict[str, any]]:
        ...
        for service_name in self.detector.service_endpoints.keys():
            ...
            try:
                with urllib.request.urlopen(health_url, timeout=5) as response:
                    if response.getcode() == 200:
                        ...
                        try:
                            ...
                            results[service_name] = {"status": "healthy", "response": health_data, "url": health_url}
                        except json.JSONDecodeError:
                            results[service_name] = {"status": "healthy", "response": content[:200], "url": health_url}
                    else:
                ...
                results[service_name] = {"status": "unreachable", "error": str(e), "url": health_url}
