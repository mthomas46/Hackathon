...
class DockerComposeGenerator:
    ...
    def generate_service_block(self, service_name, dockerfile_path, dependencies=None, profiles=None):
        ...
        service_block = {
            ...
            "environment": ["PYTHONPATH=/app", f'SERVICE_PORT={port_config["internal"]}', "ENVIRONMENT=development"],
            "ports": [f'{port_config["external"]}:{port_config["internal"]}'],
            "healthcheck": {
                "test": ["CMD", "curl", "-f", f'http://localhost:{port_config["internal"]}/health'],
                "interval": "30s",
                "timeout": "10s",
                "retries": 3,
            },
