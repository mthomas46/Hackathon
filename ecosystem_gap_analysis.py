...
class EcosystemGapAnalyzer:
    ...
    def analyze_port_mapping_discrepancies(self):
        ...
        try:
            result = subprocess.run(
                ["docker", "ps", "--format", "table {{.Names}}\t{{.Ports}}"], capture_output=True, text=True, timeout=10
            )
    ...
    def analyze_service_startup_errors(self):
        ...
        for container_name in problematic_services:
            try:
                result = subprocess.run(
                    ["docker", "logs", "--tail", "20", container_name], capture_output=True, text=True, timeout=10
                )
    ...
    def _generate_prioritized_recommendations(self) -> List[str]:
        ...
        connectivity_issues = [
            i for i in self.issues if "connectivity" in i.issue_type or "service_down" in i.issue_type
        ]
