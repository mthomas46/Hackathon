...
class PowerUserCLITester:
    ...
    def run_cli_command(self, service: str, command: str, timeout: int = 15) -> Tuple[bool, str, float]:
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, self.cli_path, service, command], capture_output=True, text=True, timeout=timeout
            )
    ...
    def test_ecosystem_health_overview(self) -> None:
        ...
        if success:
            ...
            for line in lines:
                if any(
                    keyword in line.lower() for keyword in ["health summary", "services healthy", "ecosystem status"]
                ):
                    print(f"    ↳ {line.strip()}")
        else:
            error_line = output.split("\n")[0] if output else "Unknown error"
