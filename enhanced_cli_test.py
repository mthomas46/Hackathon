...
async def test_enhanced_cli():
    ...
    test_results = {"total_tests": 0, "passed_tests": 0, "failed_tests": 0, "service_tests": {}}
    ...
    async def test_github_create_issue():
        response = await clients.post_json(
            "http://github-mcp:5072/repositories/test/repo/issues", {"title": "Test Issue", "body": "Test body"}
        )
        return response.get("success") == True
    ...
    async def test_source_process():
        response = await clients.post_json(
            "http://source-agent:5000/process", {"content": "# Test Document", "type": "markdown"}
        )
        return response.get("success") == True
    ...
    async def test_analysis_to_orchestrator():
        ...
        orchestrator_result = await clients.post_json(
            "http://orchestrator:5099/registry/services", {"service": "analysis-service", "result": analysis_result}
        )
    ...
    async def test_github_to_source():
        ...
        if isinstance(github_result, list):
            ...
            content = (
                github_result.get("content", "# Sample content from repository")
                if github_result
                else "# Sample content"
            )

        source_result = await clients.post_json(
            "http://source-agent:5000/process", {"content": content, "source": "github"}
        )
