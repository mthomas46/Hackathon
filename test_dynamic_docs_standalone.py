...
def _generate_dynamic_documents(query: str, context: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    authors = ["Sarah Johnson", "Mike Chen", "Emma Wilson", "Lisa Rodriguez", "Alex Thompson", "David Kim", "Priya Patel", "John Doe", "Jane Smith"]
    ...
    categories = ["architecture", "api", "feature", "bug", "design", "requirements", "user_story", "rfc", "meeting_notes", "testing", "deployment", "security"]
    doc_types = ["confluence", "jira", "pr", "design_doc", "user_story", "rfc", "meeting_notes", "test_plan", "deployment_guide"]
    ...
    def rand_date(days_ago=10):
        return (now - timedelta(days=random.randint(0, days_ago), hours=random.randint(0, 23), minutes=random.randint(0, 59))).isoformat()
    ...
    for i, (title, cat, content, prio, stat) in enumerate([
        (f"Implement Advanced {project_type.title()} Dashboard", "feature", f"As a product manager, I want an advanced dashboard so I can track {project_type} platform metrics.\n\n## Acceptance Criteria\n- Real-time metrics\n- {frontend} frontend\n- {backend} backend API\n- Mobile-responsive", "high", "in_progress"),
        (f"Bug: {project_type.title()} API returns 500 on POST /items", "bug", f"Steps to Reproduce:\n1. POST to /api/v1/{project_type}/items with invalid payload\n2. Observe 500 error\n\n## Expected\nGraceful validation error (400)\n\n## Actual\n500 Internal Server Error\n\n## Logs\n```\n[stacktrace]\n```", "critical", "open"),
        (f"Epic: {project_type.title()} Mobile App Launch", "feature", f"Epic to track all work for mobile app launch.\n\n## Child Issues\n- JIRA-123: Mobile UI\n- JIRA-124: API integration\n- JIRA-125: Push notifications", "high", "open"),
        (f"Task: Refactor {backend} Service Layer", "feature", f"Refactor service layer for maintainability and testability.\n\n## Subtasks\n- Add unit tests\n- Extract business logic\n- Update documentation", "medium", "review"),
        (f"Story: As a user, I can reset my password via email", "user_story", f"User should be able to request password reset and receive a secure email link.\n\n## Acceptance Criteria\n- Email sent to user\n- Link expires in 30 minutes\n- Password complexity enforced", "high", "resolved"),
    ]):
        documents.append({
